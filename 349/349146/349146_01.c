/*
 * A349146 -- ordered n-tuples with integral reciprocal sum.
 *
 ...
 *
 * A sparse dynamic program stores (number of entries used, residue).  Its
 * count is the number u!/Product c_j! of ordered words on the u entries whose
 * denominators have already been processed.  Adding c copies of a new value
 * multiplies this count by binomial(u+c,c).  The omitted value c_1 is uniquely
 * n-u; inserting these ones at the end multiplies by binomial(n,u).  Hence the
 * final weight is exactly n!/Product c_j!, the number of ordered tuples having
 ...
 * n<=66, every partial or final count is at most the number n^n of all ordered
 * n-tuples.  Its largest value in this range is
 *
 *   66^66
 ...
 * nevertheless checked at runtime.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -Werror \
 *       349146_01.c -o 349146_01
 *
 * Usage:
 *   ./349146_01
 *   ./349146_01 --upto 25 --verbose
 *   ./349146_01 --term 25 --memory-mb 1024 --verbose
 *   ./349146_01 --check
 *
 * The default and --upto modes print completed terms and atomically replace
 * b349146_01.txt.  --term and --check do not modify the b-file.
 */
...
#define BFILE_NAME "b349146_01.txt"
                                  ...
                                  Statistics *statistics)
{
            ...
            for (unsigned multiplicity = 0;
                ...
                if (!closes_prime || next_residue % prime_power == 0U) {
                    ...
                    const Count weighted = count_multiply_u64(
                        &state.count,
                        search.binomial[used + multiplicity][multiplicity]);
                    table_add(&search, &next, key, &weighted);
                }
    ...
    for (size_t slot = 0; slot < current.capacity; ++slot) {
        ...
        unsigned used = 0U;
        uint64_t residue = 0U;
        unpack_key(&search, state.key_plus_one - 1U, &used, &residue);
        if (residue != 0U)
            die("nonzero residue remains at final modulus");
        const Count completed = count_multiply_u64(
            &state.count, search.binomial[n][used]);
...
{
    fprintf(stderr,
            "349146_01: n=%d, initial_modulus=%" PRIu64
            ", peak_states=%zu, "

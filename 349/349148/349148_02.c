/*
 * A349148 -- unordered n-tuples with integral reciprocal sum (algorithm 02).
 *
 ...
 * 349148_01, this implementation has no hash table and never inserts generated
 ...
 *
 * The omitted value c_1 is determined uniquely as n minus the number used and
 * does not affect integrality, because 1/1 is integral.
 *
 ...
 *       349148_02.c -o 349148_02
 *
 * Usage:
 *   ./349148_02
 *   ./349148_02 --upto 25 --verbose
 *   ./349148_02 --term 25 --memory-mb 1024 --verbose
 *   ./349148_02 --check
 *
 * The default and --upto modes print completed terms and atomically replace
 * b349148_02.txt.  --term and --check do not modify the b-file.
 */
...
#define BFILE_NAME "b349148_02.txt"
                           ...
                           uint64_t prime_power, uint64_t new_modulus)
{
    while (stream->scanned < current->size) {
        ...
        stream->count = current->entry[index].count;
        return true;
    }
    return false;
}
                                    ...
                                    Statistics *statistics)
{
    ...
    for (size_t index = 0; index < current.size; ++index)
        count_add(&total, &current.entry[index].count);
...
{
    fprintf(stderr,
            "349148_02: n=%d, initial_modulus=%" PRIu64
            ", peak_states=%zu, "

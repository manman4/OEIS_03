/*
 * A360386 and the family a_{n,d} -- Tauraso's exact formula.
 *
 * Count permutations p of {1,...,n} satisfying
 *
 *              |p(i+8) - p(i)| != 8,   1 <= i <= n-8.
 *
 * Roberto Tauraso's Theorem 1 in
 *
 *   "The Dinner Table Problem: The Rectangular Case",
 *   INTEGERS 6 (2006), #A11,
 *   https://math.colgate.edu/~integers/g11/g11.pdf
 *
 * gives an explicit inclusion-exclusion formula for a_{n,d}.  This program
 * evaluates that formula exactly for a user-selected d>=2; d=8 is the
 * default and gives A360386.  The implementation below is a regrouping of
 * its displayed sums, not a different recurrence.
 *
 ...
 *
 * Tauraso's q_{n,d}(L) depends only on H.  If x_l components of length l are
 * sent to a target path of size m, its unlabeled contribution is
 *
 ...
 * Tauraso's formula.  It does not replace the formula by a recurrence or by
 * the Spahn--Zeilberger formula.  The older per-H bounded mixed-radix
 * convolution remains available as --direct-q for an independent check.
 * Finally the theorem becomes
 *
 *   a_{n,d} = sum_H (-1)^r 2^c (n-r-c)! W(H) q(H).
 *
 ...
 *
 * GMP is used for every quantity that can grow like n!.  The supported
 * region is 2<=d<=64 and n<=128.  A residue path can therefore have at most
 * 64 vertices.  Source multiplicities fit unsigned 128-bit arithmetic
 * because their sum is at most 2^126.  A range run writes
 * b360386_01_dD.txt.  A readable part file is flushed after every term and
 * renamed atomically after success.
 *
 * Checks:
 *   - every available OEIS term a(0)..a(20) is built in and verified;
 *   - --check also uses an independent permutation DP through n=16;
 *   - the d=2 specialization is compared with all 16 values in Table 1 of
 *     Tauraso's paper;
 ...
 *     360386_01.c -lgmp -o 360386_01
 *
 * Usage:
 *
 *   ./360386_01                 # write a(0)..a(20)
 *   ./360386_01 32              # write a(0)..a(32)
 *   ./360386_01 --term 32
 *   ./360386_01 --d 5 --term 30
 *   ./360386_01 --d 5 --upto 30
 *   ./360386_01 --direct-q --d 5 --term 30
 *   ./360386_01 --check
 */
...
#error "360386_01 requires a platform with 64-bit unsigned long"
#endif

#if !defined(__SIZEOF_INT128__)
#error "360386_01 requires compiler support for unsigned __int128"
#endif
...
#define KNOWN_MAX_N 20
#define BRUTE_CHECK_MAX_N 16
#define PAPER_D2_MAX_N 16
...
/* Table 1, column a_{n,2}, on page 3 of Tauraso's paper. */
static const char *const paper_d2_terms[PAPER_D2_MAX_N + 1] = {
    NULL,
    "1",
    "2",
    "4",
                          ...
                          bool verbose)
{
    ...
    if (d == A360386_D && n <= KNOWN_MAX_N) {
        verify_decimal(result, known_terms[n], "A360386", n);
    }
    if (verbose) {
...
static int check_implementation(int maximum_n)
{
    ...
    for (int n = 1; n <= PAPER_D2_MAX_N; ++n) {
        FormulaStats stats;
        compute_formula(value, n, 2, Q_GLOBAL, &stats);
        verify_decimal(value, paper_d2_terms[n],
                       "Tauraso Table 1, d=2", n);
    }
    ...
    mpz_clears(value, direct, NULL);
    printf("ok: global and direct-q evaluations of Tauraso's formula "
           "agree through n=%d for d=8; A360386 agrees through n=%d, "
           "with the independent DP through n=%d, and with the "
           "paper's d=2 table through n=%d; general d=2..%d was "
           ...
           maximum_n < BRUTE_CHECK_MAX_N
               ? maximum_n : BRUTE_CHECK_MAX_N,
           PAPER_D2_MAX_N, GENERAL_CHECK_MAX_D, GENERAL_CHECK_MAX_N);
    return EXIT_SUCCESS;
}
                         ...
                         QMode q_mode)
{
    ...
    for (int n = SEQUENCE_OFFSET; n <= maximum_n; ++n) {
        ...
        if (gmp_fprintf(stream, "%d %Zd\n", n, value) < 0 ||
            ...
            free(final_path);
            free(part_path);
            die("could not write the a_{n,d} output file");
        }
    }
    mpz_clear(value);
    if (fclose(stream) != 0) {
        free(final_path);
        free(part_path);
        die("could not close the a_{n,d} output file");
    }
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            "MAX_N defaults to %d and may be at most %d for every D.\n"
            "A range run writes b360386_01_dD.txt.\n"
            "--direct-q uses the slower per-histogram q convolution.\n",
            program, program, program, program, program, program, program,
            A360386_D, MAX_SUPPORTED_D, DEFAULT_MAX_N,
            MAX_SUPPORTED_N);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2 && strcmp(argv[1], "--check") == 0) {
        ...
        int maximum_n = argc == 3
                            ...
                            : DEFAULT_CHECK_N;
        validate_n_d(maximum_n, A360386_D);

/*
 * A328151: least k having exactly n ordered positive solutions of
 *
 *                    x^2 + y^2 = k.
 *
 ...
 *
 * Put D(k) = product (e_p+1), over primes p == 1 (mod 4).  If k is
 ...
 * at a prime p == 3 (mod 4) gives no representations.
 *
 ...
 * multiset.  Its smallest odd part assigns the largest e_i to 5, the next to
 * 13, then 17, 29, etc.  If all e_i are even but S=0 is required, multiplying
 * by 2 gives the least nonsquare without changing D.  Squares of inert primes
 ...
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 328151_01.c \
 *       -o 328151_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./328151_01 --target 29 --verbose
 *   ./328151_01 --upto 100
 *   ./328151_01 --self-test 1000000
 */
...
#define BFILE_NAME "b328151.txt"
#define BFILE_TEMP_NAME "b328151.txt.tmp"
...
static void answer_copy(Answer *destination, const Answer *source)
{
    ...
    destination->factor_two = source->factor_two;
...
static void consider_partition(Search *search, uint64_t d, bool axis)
{
    ...
    mpz_t value, power;
    const bool factor_two = !axis && all_even;
    mpz_init_set_ui(value, factor_two ? 2 : 1);
    mpz_init(power);
    for (unsigned i = 0; i < search->factor_count; ++i) {
        const uint64_t exponent =
    ...
    if (!search->best.found || mpz_cmp(value, search->best.value) < 0) {
        ...
        search->best.factor_two = factor_two;
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 1; y * y < limit; ++y)
        for (uint64_t x = 1; x * x <= limit - y * y; ++x)
            ++direct[x * x + y * y];
    ...
    static const Regression oeis_regressions[] = {
        {0, 0}, {1, 2}, {2, 5}, {3, 50}, {4, 65}, {5, 1250},
        {6, 325}, {7, 31250}, {8, 1105}, {9, 8450}, {10, 8125},
        {11, 19531250}, {12, 5525}, {13, 488281250}, {14, 105625},
        {15, 211250}, {16, 27625}, {17, 305175781250},
        {18, 71825}, {19, 7629394531250}, {20, 138125},
        {21, 5281250}, {22, 126953125}, {23, 4768371582031250},
        {24, 160225}, {25, 35701250}, {26, 1221025},
        {27, 2442050}, {28, 3453125}
    };
    for (size_t i = 0;
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (n != 0) {
            ...
            passed = passed && reconstructed_d == answer.d &&
                     ...
                     answer.factor_two == (!answer.axis && all_even);
        }

/*
 * A320843 -- exact sparse permanent implementation.
 *
 ...
 *
 *     A[i,j] = 1  iff  i|j or j|i.
 *
 * The number of nonzero entries is O(n log n).  In particular, after the
 * indices 1..floor(n/2) and floor(n/2)+1..n are grouped, the lower-right
 * block is an identity matrix.  Instead of a dense 2^n array, the primary
 ...
 * deliberately different from those used by both 320843_02.c and
 * 320843_03.c.  For an implementation-level cross-check of 03, this program
 * also reverses the vertex-to-bit numbering, uses a different pair hash, and
 * reconstructs by a direct GMP-based CRT sum instead of incremental CRT.
 * A rigorous product
 ...
 * the row-layer DP in 320843_02.c.
 *
 * Safety policy:
 *   - n is limited to 63 because column subsets use uint64_t masks and the
 *     full mask is formed with a defined shift strictly below 64 bits;
 *   - active and transient DP hash-table storage is limited to 2 GiB by
 *     default (override with A320843_04_MEMORY_MIB);
 *   - allocation failure and bound overflow stop with an error;
 *   - the b-file is written to a readable _part.txt file beside the
 *     executable and is atomically renamed only after successful completion.
 *
 * Useful invocations:
 *
 *   ./320843_04 --check
 *   ./320843_04 --term 50
 *   ./320843_04 35
 *   ./320843_04 50 50   # use built-in known terms 0..49; compute only 50
 *
 * The normal output file is b320843_3.txt beside the executable, regardless
...
/* Deliberately disjoint from the primes in 320843_02.c and 320843_03.c. */
...
static const char *const known_terms[KNOWN_MAX_N + 1] = {
    ...
    "28167109438114448",
    "33158989380172192",
    /* Nathan McNew, Permutations and the Divisor Graph of [1,n], Table 2. */
    "107833432035711440"
};
...
{
    const char *text = getenv("A320843_04_MEMORY_MIB");
    ...
    if (text != NULL && *text != '\0') {
        ...
        if (errno != 0 || end == text || *end != '\0' ||
            ...
            fprintf(stderr,
                    "error: A320843_04_MEMORY_MIB must be in %" PRIu64
                    "..%" PRIu64 ": %s\n",
                                      ...
                                      bool report, DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_04: n=%d, bound=%zu bits, %zu limb%s, "
                "memory limit=%.2f GiB\n",
 ...
 * branching.  Symmetry of the divisibility matrix identifies (R,C) with
...
{
    uint64_t left_hash =
        mix64(rows ^ UINT64_C(0x9e3779b97f4a7c15));
    uint64_t right_hash =
        mix64(columns ^ UINT64_C(0xbf58476d1ce4e5b9));
    return mix64(left_hash +
                 ((right_hash << 41) | (right_hash >> 23)));
}
                                ...
                                uint32_t value)
{
    ...
    for (;;) {
        ...
        if (solver->report && memo->count >= solver->next_report) {
            fprintf(stderr,
                    "320843_04: n=%d memo=%zu calls=%" PRIu64
                    " allocated=%.2f GiB\n",
                    solver->n, memo->count,
...
static uint64_t highest_set_bit(uint64_t value)
{
    return UINT64_C(1) << (63U - (unsigned)__builtin_clzll(value));
}
                               ...
                               uint64_t allowed_targets)
{
    ...
    while (vertices != 0) {
        uint64_t bit = highest_set_bit(vertices);
        vertices ^= bit;
                                  ...
                                  uint64_t columns, uint32_t *result)
{
    ...
    /* The permanent of the empty matrix is 1; one empty side is impossible. */
    if (rows == 0 || columns == 0) {
        *result = rows == 0 && columns == 0 ? 1 : 0;
        return DP_OK;
    }
    ...
    for (;;) {
        ...
        uint64_t scan = rows;
        while (scan != 0) {
            uint64_t row_bit = highest_set_bit(scan);
            scan ^= row_bit;
        ...
        scan = columns;
        while (scan != 0) {
            uint64_t column_bit = highest_set_bit(scan);
            scan ^= column_bit;
            unsigned column =
    ...
    uint64_t component_rows = highest_set_bit(rows);
    ...
    uint64_t scan = rows;
    while (scan != 0) {
        uint64_t bit = highest_set_bit(scan);
        scan ^= bit;
        unsigned index = (unsigned)__builtin_ctzll(bit);
    ...
    *result = 0;
    while (choices != 0) {
        uint64_t choice = highest_set_bit(choices);
        choices ^= choice;
                                  ...
                                  DpStats *stats)
{
    ...
    for (int i = 0; i < n; ++i) {
        uint64_t mask = 0;
        for (int j = 0; j < n; ++j) {
            /* Reverse the logical vertex-to-bit map used by 320843_03.c. */
            int left = n - i;
            int right = n - j;
    ...
    if (report) {
        fprintf(stderr,
                "320843_04: n=%d graph modular DP, p=%" PRIu32
                ", memory limit=%.2f GiB\n",
                n, modulus,
                (double)memory_budget /
                    (double)(UINT64_C(1) << 30));
                                  ...
                                  size_t modulus_count)
{
    ...
    mpz_t inverse;
    mpz_t term;
    mpz_inits(partial_product, modulus_value, inverse, term, NULL);
    mpz_set_ui(result, 0);
    bool valid = true;
    for (size_t index = 0; index < modulus_count; ++index) {
        unsigned long modulus = (unsigned long)graph_moduli[index];
        mpz_divexact_ui(partial_product, product, modulus);
        mpz_set_ui(modulus_value, modulus);
        mpz_mod(term, partial_product, modulus_value);
        if (mpz_invert(inverse, term, modulus_value) == 0) {
            valid = false;
            break;
        }
        mpz_mul_ui(term, partial_product,
                   (unsigned long)residues[index]);
        mpz_mul(term, term, inverse);
        mpz_add(result, result, term);
    }
    if (valid) {
        mpz_mod(result, result, product);
    }

    for (size_t index = 0; valid && index < modulus_count; ++index) {
    ...
    mpz_clears(term, inverse, modulus_value, partial_product, NULL);
    return valid;
}
                                ...
                                DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_04: n=%d exact graph CRT, bound=%zu bits, "
    ...
    for (size_t index = 0; index < modulus_count; ++index) {
        if (report) {
            fprintf(stderr,
                    "320843_04: n=%d CRT pass %zu/%zu, p=%" PRIu32
                    "\n",
        ...
        if (report) {
            fprintf(stderr,
                    "320843_04: n=%d pass %zu/%zu ok, residue=%" PRIu32
...
static void report_failure(int n, DpStatus status,
                           ...
                           const DpStats *stats)
{
    fprintf(stderr, "error: A320843 n=%d: %s\n",
    ...
    if (status == DP_MEMORY_LIMIT ||
        ...
        fprintf(stderr,
                "Increase A320843_04_MEMORY_MIB only if the machine has "
                "enough physical RAM.\n");
    }
}
...
static void verify_known_value(const mpz_t value, int n)
{
    ...
    if (mpz_set_str(expected, known_terms[n], 10) != 0) {
        mpz_clear(expected);
        die("invalid built-in A320843 term");
    }
                            ...
                            uint64_t memory_budget, bool report)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_04: n=%d done, peak states=%zu, "
...
static int check_known_terms(int max_n, uint64_t memory_budget)
{
    ...
    mpz_clear(layer_value);
    mpz_clear(value);
    printf("ok: graph DP agrees with the known A320843 prefix "
                           ...
                           uint64_t memory_budget)
{
    ...
    char *path = path_beside_executable(argv0, "b320843_3.txt");
    char *part =
        path_beside_executable(argv0, "b320843_3_part.txt");
    ...
    if (fflush(stream) != 0) {
        ...
        free(part);
        free(path);
        die("could not flush the built-in b-file prefix");
    }
    if (start_n > 0) {
        fprintf(stderr,
                "320843_04: using built-in verified prefix n=0..%d\n",
                start_n - 1);
    }
    ...
    for (int n = start_n; n <= max_n; ++n) {
        ...
        if (gmp_fprintf(stream, "%d %Zd\n", n, value) < 0 ||
            ...
            free(part);
            free(path);
            die("could not write the A320843 b-file");
        }
    }
    mpz_clear(value);
    ...
    if (fclose(stream) != 0) {
        free(part);
        free(path);
        die("could not close the A320843 b-file");
    }

 ...
 * deliberately different from those used by 320843_02.c.  A rigorous product
 ...
 *     default (override with A320843_MEMORY_MIB);
 ...
 *   ./320843_03 --term 50
 *   ./320843_03 35
 *   ./320843_03 50 50   # use built-in known terms 0..49; compute only 50
 *
 * The normal output file is b320843_1.txt beside the executable, regardless
...
/* Deliberately disjoint from the six primes in 320843_02.c. */
...
{
    const char *text = getenv("A320843_MEMORY_MIB");
    ...
    if (text != NULL && *text != '\0') {
        ...
        if (errno != 0 || end == text || *end != '\0' ||
            ...
            fprintf(stderr,
                    "error: A320843_MEMORY_MIB must be in %" PRIu64
                    "..%" PRIu64 ": %s\n",
                                      ...
                                      bool report, DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_03: n=%d, bound=%zu bits, %zu limb%s, "
                "memory limit=%.2f GiB\n",
                                ...
                                uint32_t value)
{
    ...
    for (;;) {
        ...
        if (solver->report && memo->count >= solver->next_report) {
            fprintf(stderr,
                    "320843_03: n=%d memo=%zu calls=%" PRIu64
                    " allocated=%.2f GiB\n",
                    solver->n, memo->count,
...
/*
 * Compare masks as if their n low bits had been reversed.  The lowest
 * differing input bit becomes the highest differing reversed bit, so no
 * explicit bit reversal is needed.  This gives the same transpose
 * canonicalization order as 320843_04.c while retaining this program's
 * independent vertex-to-bit mapping.
 */
static bool reversed_mask_greater(uint64_t left, uint64_t right)
{
    uint64_t difference = left ^ right;
    if (difference == 0) {
        return false;
    }
    uint64_t lowest_difference =
        difference & (UINT64_C(0) - difference);
    return (left & lowest_difference) != 0;
}
                                  ...
                                  uint64_t columns, uint32_t *result)
{
    ...
    if (reversed_mask_greater(rows, columns)) {
                                  ...
                                  DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_03: n=%d graph modular DP, p=%" PRIu32
                ", memory limit=%.2f GiB\n",
                n, modulus,
                (double)memory_budget /
                    (double)(UINT64_C(1) << 30));
                                ...
                                DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_03: n=%d exact graph CRT, bound=%zu bits, "
    ...
    for (size_t index = 0; index < modulus_count; ++index) {
        if (report) {
            fprintf(stderr,
                    "320843_03: n=%d CRT pass %zu/%zu, p=%" PRIu32
                    "\n",
        ...
        if (report) {
            fprintf(stderr,
                    "320843_03: n=%d pass %zu/%zu ok, residue=%" PRIu32
                           ...
                           const DpStats *stats)
{
    ...
    if (status == DP_MEMORY_LIMIT ||
        ...
        fprintf(stderr,
                "Increase A320843_MEMORY_MIB only if the machine has "
                "enough physical RAM.\n");
    }
}
                            ...
                            uint64_t memory_budget, bool report)
{
    ...
    if (report) {
        fprintf(stderr,
                "320843_03: n=%d done, peak states=%zu, "
                           ...
                           uint64_t memory_budget)
{
    ...
    char *path = path_beside_executable(argv0, "b320843_1.txt");
    char *part =
        path_beside_executable(argv0, "b320843_1_part.txt");
    ...
    if (start_n > 0) {
        fprintf(stderr,
                "320843_03: using built-in verified prefix n=0..%d\n",
                start_n - 1);
    }

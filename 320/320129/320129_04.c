/*
 * A320129 -- vertex inclusion-exclusion in Gray-code order.
 *
 * For V={1,...,2*n}, let
 *
 *   c_s(S) = #{ {x,y} subset S : x<y and x+y=s }.
 *
 * If e_n denotes the n-th elementary symmetric polynomial, then
 *
 *   a(n) = sum_{S subset V} (-1)^(2*n-|S|) e_n(c_3(S),c_4(S),...).
 *
 * Here e_n chooses n pairs having distinct sums.  Vertex
 * inclusion-exclusion removes every choice whose union is not V.  Since n
 * pairs have exactly 2*n element occurrences, a choice whose union is V is
 * automatically a partition into disjoint pairs.
 *
 * Subsets are visited in binary-reflected Gray-code order.  When one element
 * v is toggled, only pairs {v,y} with y in the remaining subset change.  If
 * one factor (1+old*t) changes to (1+new*t), the elementary-symmetric
 * coefficients are updated exactly by dividing by the old linear factor and
 * multiplying by the new one, both truncated after degree n.
 *
 ...
 *
 *        2^(2*n-1) * binomial(binomial(2*n,2), n)
 *
 * is checked to fit U128.  It bounds each sign accumulator.  The final answer
 * is checked against the unrestricted pair-partition count (2*n-1)!!.  All
 * arithmetic that contributes to the answer is checked.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       320129_04.c -o 320129_04
 *
 * Usage:
 *   ./320129_04 --term 13 --threads 4
 *   ./320129_04 --upto 13 --start 13 --threads 4
 *   ./320129_04 --check 12 --threads 4
 *
 * A positional N is shorthand for --upto N.  --upto writes b320129_4.txt
 * beside the executable through interruption-safe b320129_4_part.txt.
...
#error "320129_04 requires unsigned __int128"
#endif
...
static U128 accumulator_upper_bound(unsigned element_count,
                                    unsigned pair_count, int n)
{
    if (element_count == 0) {
        return 1;
    }
    U128 selections = choose_u128(pair_count, (unsigned)n);
...
static Pair *make_pairs(unsigned element_count, unsigned *pair_count)
{
    *pair_count = element_count * (element_count - 1U) / 2U;
    Pair *pairs = calloc(*pair_count, sizeof(*pairs));
    if (pairs == NULL) {
        die("cannot allocate pair list");
    }
    unsigned index = 0;
    ...
    if (index != *pair_count) {
        die("internal pair-list size mismatch");
    }
    return pairs;
}
...
static void initialize_counts_and_polynomial(
    ...
    unsigned counts[MAX_PAIR_SUM + 1],
    U128 elementary[MAX_N + 1])
{
    memset(counts, 0, (MAX_PAIR_SUM + 1) * sizeof(*counts));
    for (unsigned index = 0; index < queue->pair_count; ++index) {
        const Pair *pair = &queue->pairs[index];
        if ((pair->bits & ~subset) == 0) {
            ++counts[pair->sum];
        }
    }
    ...
    U128 elementary[MAX_N + 1], WorkerStats *stats)
{
    ...
    while (remaining != 0) {
        uint64_t partner_bit = remaining & (UINT64_C(0) - remaining);
        unsigned partner =
            (unsigned)__builtin_ctzll(partner_bit) + 1U;
        unsigned sum = element + partner;
        unsigned old_count = counts[sum];
        unsigned new_count;
        if (adding) {
            if (old_count == UINT_MAX) {
                die("pair-sum count overflow");
            }
            new_count = old_count + 1U;
        } else {
            if (old_count == 0) {
                die("internal negative pair count");
            }
            new_count = old_count - 1U;
        }
        ...
        remaining ^= partner_bit;
        add_u64_saturating(&stats->partners_examined, 1);
        add_u64_saturating(&stats->changed_sum_factors, 1);
    }
                            ...
                            bool final)
{
    ...
    fprintf(stderr,
            terminal ? "\r320129_04: n=%d progress subsets=%" PRIu64
                       "/%" PRIu64 " (%5.1f%%), rate=%.2f M/s, "
                       "elapsed=%.1f s"
                     : "320129_04: n=%d progress subsets=%" PRIu64
                       "/%" PRIu64 " (%5.1f%%), rate=%.2f M/s, "
                       "elapsed=%.1f s\n",
                             ...
                             unsigned *actual_workers)
{
    ...
    uint64_t unrestricted_bound = pairing_upper_bound(n);
    ...
    *actual_workers = 1;
    ...
    Pair *pairs = make_pairs(element_count, &pair_count);
    U128 sign_bound = accumulator_upper_bound(
        element_count, pair_count, n);
    ...
    TaskQueue queue = {
        ...
        .maximum_sum = 2U * element_count - 1U,
    ...
    for (unsigned index = 0; index < workers; ++index) {
        ...
        add_u64_saturating(&stats->partners_examined,
                           worker[index].stats.partners_examined);
    ...
    if (answer > unrestricted_bound || answer > UINT64_MAX) {
        die("A320129 result exceeded the unrestricted partition bound");
    }
                                ...
                                unsigned progress_seconds, bool report)
{
    ...
    if (report) {
        fprintf(stderr,
                "320129_04: n=%d, Gray-code inclusion-exclusion, "
                "subsets=%" PRIu64 ", partners=%" PRIu64
                ", factor-updates=%" PRIu64 ", %u worker%s, %.3f s\n",
                n, stats.subsets, stats.partners_examined,
                           ...
                           unsigned progress_seconds)
{
    ...
    char *final_path = path_beside_executable(argv0, "b320129_4.txt");
    char *part_path =
        path_beside_executable(argv0, "b320129_4_part.txt");
    ...
    if (start_n > MIN_N) {
        fprintf(stderr,
                "320129_04: using built-in verified prefix n=%d..%d\n",
                MIN_N, start_n - 1);
    }
...
int main(int argc, char **argv)
{
    ...
    if (mode == MODE_TERM) {
        ...
        printf("ok: Gray-code inclusion-exclusion agrees with A320129 for "
               "n=%d..%d\n", MIN_N, target);
    }
    return EXIT_SUCCESS;
}

/*
 * A320129 -- bounded memoized and parallel exact search.
 *
 ...
 *
 * The smallest unused element is paired with every admissible partner.
 * Completed state values are stored in a bounded direct-mapped memo table.
 * Both key words are compared before a cached value is used.  A hash
 * collision merely replaces an older entry and can only cause recomputation;
 * it cannot change the answer.  The fixed cache makes memory use predictable.
 *
 * Root branches (the partner of element 1) are distributed among pthread
 * workers.  Each worker owns a private memo table, so there are no cache data
 * races and thread scheduling affects performance only.  Every count addition
 * is checked.  Before searching, (2*n-1)!! is checked to fit uint64_t, which
 * rigorously bounds the answer and every subproblem value.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       320129_02.c -o 320129_02
 *
 * Usage:
 *   ./320129_02 --term 10 --threads 4 --cache-mib 256
 *   ./320129_02 --upto 10 --threads 4 --cache-mib 256
 *   ./320129_02 --check 8 --threads 4 --cache-mib 64
 *
 * A positional N is shorthand for --upto N.  --upto writes b320129_2.txt
 * beside the executable through interruption-safe b320129_2_part.txt.
...
#error "320129_02 requires unsigned __int128"
#endif
...
/* high_tag is key.high+1, so zero denotes an empty slot. */
typedef struct {
    uint64_t low;
    uint64_t high_tag;
    uint64_t value;
...
typedef struct {
    ...
    _Atomic unsigned next_branch;
    uint64_t *branch_values;
} RootQueue;
...
{
    return mix64(key.low ^ mix64(key.high + UINT64_C(0x9e3779b97f4a7c15)));
}

static StateKey make_key(const Search *search, uint64_t unused,
                         uint64_t used_sums)
{
    U128 packed = (U128)unused |
                  ((U128)(used_sums >> 3) << search->element_count);
    StateKey key = {
        .low = (uint64_t)packed,
        .high = (uint64_t)(packed >> 64)
    };
    return key;
}
...
static bool memo_get(const Memo *memo, StateKey key, uint64_t *value)
{
    const MemoEntry *entry = &memo->entries[key_hash(key) & memo->mask];
    if (entry->high_tag == key.high + 1 && entry->low == key.low) {
        *value = entry->value;
        return true;
    }
    return false;
...
static void memo_put(Memo *memo, StateKey key, uint64_t value,
                     SearchStats *stats)
{
    MemoEntry *entry = &memo->entries[key_hash(key) & memo->mask];
    if (entry->high_tag != 0 &&
        (entry->high_tag != key.high + 1 || entry->low != key.low)) {
        increment_stat(&stats->replacements);
    }
    entry->low = key.low;
    entry->value = value;
    entry->high_tag = key.high + 1;
    increment_stat(&stats->stores);
}

static uint64_t count_state(Search *search, uint64_t unused,
                            uint64_t used_sums)
{
    ...
    if ((partners & (partners - 1)) == 0) {
        unsigned partner = (unsigned)__builtin_ctzll(partners) + 1U;
        return (used_sums & (UINT64_C(1) << (first + partner))) == 0;
    }

    StateKey key = make_key(search, unused, used_sums);
    uint64_t cached = 0;
    ...
    while (partners != 0) {
        ...
        uint64_t sum_bit = UINT64_C(1) << (first + partner);
        if ((used_sums & sum_bit) != 0) {
            increment_stat(&search->stats.duplicate_sum_rejections);
        } else {
            uint64_t addend = count_state(
                search, unused ^ first_bit ^ partner_bit,
                used_sums | sum_bit);
            if (answer > UINT64_MAX - addend) {
                die("A320129 count overflowed uint64_t");
            }
            answer += addend;
        }
        partners ^= partner_bit;
    }
    memo_put(search->memo, key, answer, &search->stats);
    return answer;
}
...
static void *worker_main(void *argument)
{
    ...
    for (;;) {
        unsigned branch = atomic_fetch_add_explicit(
            &queue->next_branch, 1U, memory_order_relaxed);
        if (branch >= queue->branch_count) {
            break;
        }
        unsigned partner = branch + 2U;
        uint64_t first_bit = UINT64_C(1);
        uint64_t partner_bit = UINT64_C(1) << (partner - 1U);
        uint64_t unused = queue->full_mask ^ first_bit ^ partner_bit;
        uint64_t used_sums = UINT64_C(1) << (1U + partner);
        queue->branch_values[branch] = count_state(
            &worker->search, unused, used_sums);
                            ...
                            bool final)
{
    ...
    double percent = branch_count == 0
                         ...
                         : 100.0 * (double)completed /
                               (double)branch_count;
    fprintf(stderr,
            terminal ? "\r320129_02: n=%d progress roots=%u/%u "
                       "(%5.1f%%), calls=%" PRIu64
                       ", rate=%.2f M/s, elapsed=%.1f s"
                     : "320129_02: n=%d progress roots=%u/%u "
                       "(%5.1f%%), calls=%" PRIu64
                       ", rate=%.2f M/s, elapsed=%.1f s\n",
...
static uint64_t pairing_upper_bound(int n)
{
    ...
    for (unsigned factor = 1; factor < 2U * (unsigned)n;
        ...
        bound *= factor;
        if (bound > UINT64_MAX) {
            die("total pairing bound exceeds uint64_t");
        }
    }
    return (uint64_t)bound;
}

static uint64_t compute_term(int n, int requested_threads,
                             ...
                             int *actual_workers)
{
    if (n < MIN_N || n > MAX_N || 2 * n >= 64 || 4 * n - 1 >= 64) {
        die("unsupported dimension in memoized pairing search");
    }
    ...
    *actual_workers = 1;
    ...
    unsigned branch_count = 2U * (unsigned)n - 1U;
    int workers = requested_threads;
    if (workers > (int)branch_count) {
        workers = (int)branch_count;
    }
    if (workers < 1) {
        workers = 1;
    }
    ...
    uint64_t *branch_values = calloc(branch_count, sizeof(*branch_values));
    Worker *worker = calloc((size_t)workers, sizeof(*worker));
    pthread_t *threads = calloc((size_t)workers, sizeof(*threads));
    if (branch_values == NULL || worker == NULL || threads == NULL) {
        free(threads);
        free(worker);
        free(branch_values);
    ...
    uint64_t full_mask = (UINT64_C(1) << (2 * n)) - UINT64_C(1);
    RootQueue queue = {
        .n = n,
        .full_mask = full_mask,
    ...
    for (int index = 0; index < workers; ++index) {
        worker[index].search.n = n;
        worker[index].search.element_count = 2U * (unsigned)n;
        worker[index].search.full_mask = full_mask;
    ...
    while (atomic_load_explicit(&progress.completed_branches,
                                memory_order_acquire) < branch_count) {
        double now = monotonic_seconds();
        if (progress_seconds != 0 && now >= next_report) {
            report_progress(n, &progress, branch_count, progress_start,
    ...
    uint64_t answer = 0;
    for (unsigned branch = 0; branch < branch_count; ++branch) {
        if (answer > UINT64_MAX - branch_values[branch]) {
            die("root sum overflowed uint64_t");
        }
        answer += branch_values[branch];
    }
    for (int index = 0; index < workers; ++index) {
#define ADD_STAT(field)                                                    \
        do {                                                               \
            uint64_t add = worker[index].search.stats.field;               \
            combined_stats->field =                                       \
                combined_stats->field > UINT64_MAX - add                   \
                    ? UINT64_MAX                                           \
                    : combined_stats->field + add;                         \
        } while (0)
        ADD_STAT(calls);
        ADD_STAT(hits);
        ADD_STAT(stores);
        ADD_STAT(replacements);
        ADD_STAT(duplicate_sum_rejections);
#undef ADD_STAT
...
static uint64_t compute_checked(int n, int requested_threads,
                                unsigned cache_mib,
                           ...
                           unsigned progress_seconds)
{
    ...
    char *final_path = path_beside_executable(argv0, "b320129_2.txt");
    char *part_path =
        path_beside_executable(argv0, "b320129_2_part.txt");
    ...
    if (start_n > MIN_N) {
        fprintf(stderr,
                "320129_02: using built-in verified prefix n=%d..%d\n",
                MIN_N, start_n - 1);
    }
    for (int n = start_n; n <= maximum_n; ++n) {
        uint64_t value = compute_checked(
            n, requested_threads, cache_mib, progress_seconds, true);
        if (fprintf(output, "%d %" PRIu64 "\n", n, value) < 0) {
...
int main(int argc, char **argv)
{
    ...
    if (mode == MODE_TERM) {
        ...
        printf("ok: bounded memoized search agrees with A320129 for "
               "n=%d..%d\n", MIN_N, target);
    }
    return EXIT_SUCCESS;
}

/*
 * A354756 -- independent modular verifier.
 *
 ...
 * 354756_02.c: a Johnson-Gentleman-style, row-by-row subset DP.  A state is
 ...
 * value.  The intended verification is exact-to-exact: 03 never consumes
 * 02's result while computing.  --check compares independently reconstructed
 ...
 * can be changed with A354756_03_MEMORY_MIB.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       -I/opt/homebrew/opt/gmp/include -L/opt/homebrew/opt/gmp/lib \
 *       354756_03.c -lgmp -o 354756_03
 *
 * Examples:
 *   ./354756_03 --term 35
 *   ./354756_03 --crt 35
 *   ./354756_03 --mod 35
 *   ./354756_03 --verify 38 247857779387904
 *   ./354756_03 --check 35
 *   ./354756_03 51 39
 *
 * Normal execution writes b354756_2.txt beside the executable, regardless
 * of the current working directory.  --term result lines are exact
 * b-file-style pairs:
 *
 *   n  a(n)
 *
                                  ...
                                  uint32_t *result, DpStats *stats)
{
    ...
    for (int layer = 0; layer < n; ++layer) {
        ...
        if (report) {
            fprintf(stderr,
                    "354756_03: n=%d layer=%d/%d states=%zu "
                    "memory=%.3f GiB\n",
                                ...
                                size_t modulus_count, uint32_t *residues)
{
    ...
    if (status != DP_OK) {
        fprintf(stderr,
                "354756_03: n=%d failed: %s; peak/required %.3f GiB\n",
    ...
    fprintf(stderr,
            "354756_03: n=%d ok, moduli=%zu, peak states=%zu, "
                        ...
                        uint32_t *residues)
{
    for (size_t index = 0; index < FINGERPRINT_MODULUS_COUNT; ++index) {
        fprintf(stderr,
                "354756_03: n=%d modular pass %zu/%d, p=%" PRIu32 "\n",
...
{
    const char *setting = getenv("A354756_03_MEMORY_MIB");
    ...
    if (setting != NULL && *setting != '\0') {
        ...
        if (errno != 0 || end == setting || *end != '\0' ||
            ...
            fprintf(stderr,
                    "error: A354756_03_MEMORY_MIB must be from %" PRIu64
                    " to %" PRIu64 "\n",
...
static int check_known(int max_n, uint64_t memory_budget)
{
    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        if (mpz_cmp(actual, expected) != 0) {
            fprintf(stderr, "354756_03: mismatch at n=%d\n", n);
    ...
    mpz_clears(actual, expected, NULL);
    printf("verified independently reconstructed exact A354756 terms "
           "for n=0..%d\n", max_n);
    return EXIT_SUCCESS;
}
                          ...
                          uint64_t memory_budget)
{
    ...
    if (!same_residues(actual, expected)) {
        fprintf(stderr, "354756_03: VALUE does not match at n=%d\n", n);
        return EXIT_FAILURE;
    }
                             ...
                             uint64_t memory_budget, bool report)
{
    ...
    fprintf(stderr,
            "354756_03: n=%d CRT exact mode, bound=%zu bits, "
    ...
    for (size_t index = 0; index < modulus_count; ++index) {
        fprintf(stderr,
                "354756_03: n=%d CRT pass %zu/%zu, p=%" PRIu32 "\n",
...
static bool write_known(FILE *stream, int n)
{
    if (n < 0 || n > KNOWN_MAX_N) {
        return false;
    }
    return fprintf(stream, "%d %s\n", n, known_terms[n]) >= 0;
}

static int produce_b_file(const char *argv0, int max_n, int start_n,
                          uint64_t memory_budget)
{
    ...
    char *path = path_beside_executable(argv0, "b354756_2.txt");
    ...
    for (int n = 0; n < start_n && n <= max_n; ++n) {
        if (!write_known(stream, n)) {
            status = EXIT_FAILURE;
            break;
        }
    }
    if (status == EXIT_SUCCESS && start_n > 0) {
        fprintf(stderr,
                "354756_03: using built-in verified prefix n=0..%d\n",
                start_n - 1);
    }

    mpz_t value;
    mpz_init(value);
    for (int n = start_n; status == EXIT_SUCCESS && n <= max_n; ++n) {
        if (compute_crt_value(value, n, memory_budget, true) !=
            EXIT_SUCCESS) {
            status = EXIT_FAILURE;
            break;
        }
    ...
    if (status == EXIT_SUCCESS && rename(part, path) != 0) {
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            "       %s --check [MAX_N]\n"
            "\n"
            "N, MAX_N, and START_N must be from 0 to %d.\n"
            "MAX_N defaults to %d. Terms below START_N are copied from\n"
            ...
            "A354756_03_MEMORY_MIB.\n",
            program, program, program, program, program, program,
            MAX_SUPPORTED_N, DEFAULT_MAX_N, KNOWN_MAX_N,
...
int main(int argc, char **argv)
{
    ...
    return produce_b_file(argv[0], max_n, start_n, memory_budget);
}

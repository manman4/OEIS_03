/*
 * A398904 -- deliberately naive exact counter for partitions of
 * {1,...,3*n} into
 ...
 *       398904_01.c -o 398904_01
 *
 * Usage:
 *   ./398904_01 5
 *   ./398904_01 --term 5
 *   ./398904_01 --check --no-bfile
 *
 * A positional N computes a(0),...,a(N).  Completed terms are atomically
 * recorded in b398904_01.txt by default.  --term N requires a complete
...
#error "398904_01.c requires unsigned __int128"
#endif
...
static const char *output_path = "b398904_01.txt";
...
static void record_term(int n, U128 value)
{
    ...
    fprintf(stderr, "398904_01: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static void heartbeat_if_due(void)
{
    ...
    fprintf(stderr, "398904_01: heartbeat n=%d, calls=", active_n);
    print_calls(stderr);
    fprintf(stderr, ", %.1f s\n", now - started);
...
static U128 compute_term(int n)
{
    ...
    fprintf(stderr,
            "398904_01: n=%d, naive backtracking, rows=%" PRIu32
...
{
    static const uint64_t expected[] = {1, 1, 3, 38, 809, 28253};
    for (int n = 0; n <= 5; ++n) {
    ...
    fprintf(stderr, "398904_01: self-check passed for n=0..5\n");
}
...
static void usage(FILE *stream, const char *program)
{
    fprintf(stream,
            ...
            "  --output FILE use FILE instead of b398904_01.txt\n"

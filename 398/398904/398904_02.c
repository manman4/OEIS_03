 ...
 *       398904_02.c -o 398904_02
 *
 * Usage:
 *   ./398904_02 10
 *   ./398904_02 --term 10 --hash-power 22
 *   ./398904_02 --check
 *
 * Completed terms are atomically recorded in b398904_02.txt.  --term N is
...
#error "398904_02.c requires unsigned __int128"
#endif
...
static const char *output_path = "b398904_02.txt";
...
static void record_term(int n, U128 value)
{
    ...
    free(temporary);
    unlock_bfile(lock_fd);
    fprintf(stderr, "398904_02: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static void *monitor_main(void *argument)
{
    ...
    while (!monitor_finished) {
        ...
        if (!monitor_finished && error == ETIMEDOUT) {
            fprintf(stderr,
                    "398904_02: heartbeat n=%d, tasks=%u/%u, "
                    "calls>=%" PRIu64 ", %.1f s\n",
                    monitor->n,
...
static U128 sequence_term(int n)
{
    ...
    fprintf(stderr,
            "398904_02: n=%d, sparse DP, maximum-first, rows=%u, "
...
int main(int argc, char **argv)
{
    ...
    if (check) fprintf(stderr, "398904_02: self-check passed for n=0..9\n");
    return EXIT_SUCCESS;
}

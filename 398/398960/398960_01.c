/*
 * A398960 -- bitset exact-cover search with shared value constraints.
 *
 * Count pairings {b(i),c(i)} of {1,...,2*n}, b(i)<c(i), such that all 2*n
 * positive values
 *
 *     b(i)+c(i)-1 and c(i)-b(i)
 *
 * are different.  In zero-based notation x=b-1 and y=c-1, a candidate pair
 * {x,y}, x<y, covers four columns:
 *
 *     vertex x, vertex y, value x+y+1, value y-x.
 *
 * Vertex columns are primary (each must be covered exactly once).  The
 * shifted sums and differences use one shared family of secondary value
 * columns (each may be covered at most once).  Two candidate edges conflict
 * if they share an endpoint or if either value of one edge equals either
 * value of the other.  Every edge's conflict set is precomputed as a bitset;
 * selecting edge e is then
 *
 ...
 *       398960_01.c -o 398960_01
 *
 * Usage:
 *   ./398960_01 --term 13 --threads 4
 *   ./398960_01 --upto 13 --threads 4
 *   ./398960_01 --upto 13 --start 13 --threads 4
 *   ./398960_01 --check 13 --threads 4
 *
 * A positional N is shorthand for --upto N.  --upto writes
 * b398960_01.txt through b398960_01_part.txt unless --no-bfile is given.
...
#error "398960_01.c requires unsigned __int128"
#endif
...
#define BFILE_NAME "b398960_01.txt"
#define PART_FILE_NAME "b398960_01_part.txt"
#define LOCK_FILE_NAME "b398960_01.txt.lock"
...
static Problem *make_problem(unsigned n)
{
    ...
    const unsigned maximum_value = 2 * problem->vertex_count - 2;
    ...
    for (unsigned x = 0; x < problem->vertex_count; ++x) {
        for (unsigned y = x + 1; y < problem->vertex_count; ++y) {
            ...
            if (edge->shifted_sum == edge->difference ||
                edge->shifted_sum > maximum_value ||
            ...
            set_edge_bit(row(problem->incidence, y,
                             problem->word_count), edge_index);
            set_edge_bit(row(value_edges, edge->shifted_sum,
                             problem->word_count), edge_index);
    ...
    for (unsigned e = 0; e < problem->edge_count; ++e) {
        ...
        const uint64_t *same_shifted_sum_value = const_row(
            value_edges, edge->shifted_sum, problem->word_count);

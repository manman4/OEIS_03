 ...
 * B.  A next-prime lookup table skips composite target sums, and a dynamic
 * pthread work queue distributes the complementary masks among workers.
                                 ...
                                 unsigned depth)
{
    ...
    if (rest_size >= MITM_MIN_REST_SIZE) {
        ...
        for (;;) {
            ...
            for (unsigned target =
                     shared->next_prime[pivot_value + low_value];
...
static void initialize_tables(Shared *shared)
{
    ...
    shared->next_prime[MAX_SUM + 1U] = (uint16_t)next;
    for (unsigned value = MAX_SUM + 1U; value-- > 0;) {
        if (shared->prime[value]) next = value;
        shared->next_prime[value] = (uint16_t)next;
    }
}

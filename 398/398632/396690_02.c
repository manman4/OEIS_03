/*
 * q target-assignment convolution for
 *
 ...
 * evaluates the same q target-assignment formula as 398599_02.c, with
 * permutation size N=n+k and forbidden difference s=n.
 *
 * Apply inclusion-exclusion to selected adjacency constraints.  Let h_l
 * count selected components with l edges, r=sum l*h_l, and c=sum h_l.
 * Their source multiplicity is
 *
 *       W(H) = (N-r)_c / product_l h_l!.
 *
 * Split the values 1,...,N into their n residue paths.  Assigning local
 * component histograms to those paths and convolving gives Q(H), while
 * Tauraso's q quantity is
 *
 *       q(H) = product_l h_l! * Q(H).
 *
 * The answer is evaluated as
 *
 *   sum_H (-1)^r 2^c (N-r-c)! W(H) q(H).
 *
 * This differs from algorithm 03: 02 constructs the target assignments
 * Q(H), whereas 03 constructs the common tile-partition coefficients
 * C_alpha(N,n).  The normal calculation does not enumerate permutations.
 * A small independent subset DP is used only by --check.
 *
 ...
 *     396690_02.c -lgmp -o 396690_02
 *
 * Usage:
 *
 *   ./396690_02                         # k=4, a(0)..a(6)
 *   ./396690_02 --k 4 --upto 6
 *   ./396690_02 --k 4 --term 6
 *   ./396690_02 --check                 # k=4, n=0..6
 *   ./396690_02 --check 8
 *
...
#error "396690_02 requires a platform with 64-bit unsigned long"
#endif

#if !defined(__SIZEOF_INT128__)
#error "396690_02 requires unsigned __int128"
#endif
...
static uint64_t target_multiplicity(
    ...
    mpz_t numerator, mpz_t divisor)
{
    if (available < component_count || component_count < 0 ||
        ...
        die("invalid target multiplicity arguments");
    }
    ...
    mpz_t numerator, mpz_t divisor)
{
    if (length >= path_size) {
        ...
        uint64_t coefficient = target_multiplicity(
    ...
    const uint128_t factorial[MAX_COMPONENT_COUNT + 1])
{
    ...
    if (exponent < 0 || exponent >= 64 ||
        ...
        die("target coefficients do not sum to 2^(size-step)");
    }
}
...
static void compute_q_formula(mpz_t result, int size, int step,
                              size_t *histogram_count)
{
    ...
    if (step <= 0 || size > MAX_SUPPORTED_TOTAL_SIZE) {
        die("invalid q-formula arguments");
    }
    ...
    TermList target;
    target_coefficients(&target, size, step, factorial);
    *histogram_count = target.count;
    ...
    for (size_t i = 0; i < target.count; ++i) {
        ...
        uint128_t hist_factor = histogram_factorial(
            target.item[i].count, key_length, factorial,
        ...
        uint64_t source = target_multiplicity(
            size - edge_count, component_count, hist_factor,
            factorial, term, factor);

        mpz_set_ui(term, (unsigned long)target.item[i].coefficient);
        multiply_by_u128(term, hist_factor, factor);
        multiply_by_u128(term, (uint128_t)source, factor);
...
static void compute_value(mpz_t result, int n, int k, bool verbose)
{
    ...
    if (n == 0 || k == 0) {
        ...
        compute_q_formula(result, size, n, &histogram_count);
    }

    verify_known_value(result, n, k);
    if (verbose) {
        fprintf(stderr,
                "396690_02: k=%d, n=%d, size=%d, "
                "forbidden difference=%d, target histograms=%zu, "
                "q target-assignment convolution, %.3f s\n",
...
static int check_implementation(int maximum_n)
{
    ...
    printf("ok: q target-assignment convolution agrees with built-in "
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            "Normal calculation uses q target-assignment convolution.\n"

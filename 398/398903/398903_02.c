/*
 * A398903 -- fast exact counter for partitions of {1,...,3*n} into
 ...
 *
 * The valid triples form the rows of an exact-cover matrix.  At each state,
 * the search chooses the smallest remaining value and tries every compatible
 * row containing it.  A complete partition contains exactly one selected row
 * covering that value, so induction proves that every unordered partition is
 * counted once.  This fixed choice also maximizes reuse of identical masks.
 *
 ...
 * not probabilistic hashing.  One row is expanded before the dynamically
 * scheduled parallel work queue.
 *
 ...
 * Hence the other 2*k values must have total sum at most the sum of the k
 * largest values.  The smallest capacity must also accommodate the two
 * smallest addends, and all addends must be pairable below the largest
 * capacity; pairing smallest with largest tests the latter condition.  These
 * are necessary conditions only.  Failure certifies that the state has no
 * completion; passing never contributes without the exact-cover recursion.
 *
 ...
 *       398903_02.c -o 398903_02
 *
 * Usage:
 *   ./398903_02 10
 *   ./398903_02 --term 10 --hash-power 22
 *   ./398903_02 --check
 *
 * Completed terms are atomically recorded in b398903_02.txt.  --term N is
...
#error "398903_02.c requires unsigned __int128"
#endif
...
static const char *output_path = "b398903_02.txt";
...
static void record_term(int n, U128 value)
{
    ...
    free(temporary);
    unlock_bfile(lock_fd);
    fprintf(stderr, "398903_02: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static int smallest_column(uint64_t mask, uint32_t *compatible_count)
{
    if (mask == 0) {
        *compatible_count = 0;
        return -1;
    }
    const unsigned value = (unsigned)__builtin_ctzll(mask);
...
static U128 count_state(uint64_t mask, Memo *memo, SearchStats *stats)
{
    ...
    uint32_t compatible;
    const int column = smallest_column(mask, &compatible);
...
static void build_root_tasks(uint64_t full, RootSchedule *schedule)
{
    ...
    const int root_column = smallest_column(full, &root_count);
...
static void *monitor_main(void *argument)
{
    ...
    while (!monitor_finished) {
        ...
        if (!monitor_finished && error == ETIMEDOUT) {
            fprintf(stderr,
                    "398903_02: heartbeat n=%d, tasks=%u/%u, "
                    "calls>=%" PRIu64 ", %.1f s\n",
                    monitor->n,
...
static U128 sequence_term(int n)
{
    ...
    fprintf(stderr,
            "398903_02: n=%d, sparse DP, smallest-column, rows=%u, "
...
static const char *const known[] = {
    "1", "1", "3", "28", "487", "14077", "593646", "35285047",
    "2758658973"
};
...
int main(int argc, char **argv)
{
    ...
    if (check) fprintf(stderr, "398903_02: self-check passed for n=0..8\n");
    return EXIT_SUCCESS;
}

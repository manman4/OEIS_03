 ...
 *
 * This differs from the smallest-element recurrence in 398903_02.c.  At each
 ...
 * its reads.  Sequentially consistent slot operations make that snapshot
 * rule valid even on weakly ordered CPUs and exclude ABA replacement races.
 * A collision or busy slot is just a cache miss.  Hashing therefore affects
 ...
 *       398903_03.c -o 398903_03
 *
 * Usage:
 *   ./398903_03 10
 *   ./398903_03 --term 10 --threads 8 --hash-power 24
 *   ./398903_03 --term 11 --threads 8 --hash-power 25
 *   ./398903_03 --check
 *
 * Completed terms are atomically recorded in b398903_03.txt.  --term N is
...
#error "398903_03.c requires unsigned __int128"
#endif
...
static const char *output_path = "b398903_03.txt";
...
static void record_term(int n, U128 value)
{
    ...
    free(temporary);
    unlock_bfile(lock_fd);
    fprintf(stderr, "398903_03: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static void *monitor_main(void *argument)
{
    ...
    while (!monitor_finished) {
        ...
        if (!monitor_finished && error == ETIMEDOUT) {
            fprintf(stderr,
                    "398903_03: heartbeat n=%d, tasks=%u/%u, "
                    "calls>=%" PRIu64 ", %.1f s\n",
                    monitor->n,
...
static U128 sequence_term(int n)
{
    ...
    fprintf(stderr,
            "398903_03: n=%d, sparse DP, maximum-first, rows=%u, "
...
static const char *const known[] = {
    ...
    "2758658973", "278665376055", "34901306318296"
};
...
int main(int argc, char **argv)
{
    ...
    if (check) fprintf(stderr, "398903_03: self-check passed for n=0..8\n");
    return EXIT_SUCCESS;
}

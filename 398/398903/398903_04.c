/*
 * A398903 -- Gray-code vertex inclusion-exclusion, exact modulo 2^128.
 *
 ...
 *
 * The inequality x+y<=z is not invariant under reversal, so no reflection
 * reduction is claimed or used.
 *
 ...
 * desired count is nonnegative and at most that bound, a complete residue is
 ...
 *       398903_04.c -o 398903_04
 *
 * Usage:
 *   ./398903_04 --term 9 --threads 8
 *   ./398903_04 --term 12 --threads 8 --split 12
 *   ./398903_04 --term 14 --threads 8 --split 15 --slice 0 8
 *   ./398903_04 --check 8 --threads 8
 *   ./398903_04 --upto 9 --threads 8
 *
 ...
 * to b398903_04_part.txt with fsync, and atomically renames it to
 * b398903_04.txt only after the requested prefix is complete.  An existing
 * longer b-file is never replaced by a shorter requested prefix.
 */
...
#error "398903_04 requires unsigned __int128"
#endif
...
static bool parse_decimal_u128(const char *text, U128 *result)
{
...
static void verify_complete_result(int n, U128 value)
{
    ...
    if (value > bound) {
        die("A398903 result exceeds the unrestricted partition bound");
    }
    if (n <= KNOWN_MAX_N && value != (U128)known_terms[n]) {
        fprintf(stderr, "error: A398903 mismatch at n=%d: got ", n);
                            ...
                            bool terminal, bool final)
{
    ...
    fprintf(stderr,
            terminal ? "\r398903_04: n=%d progress subsets=%" PRIu64
                       "/%" PRIu64 " (%5.1f%%), rate=%.2f M/s, %.1f s"
                     : "398903_04: n=%d progress subsets=%" PRIu64
                       "/%" PRIu64 " (%5.1f%%), rate=%.2f M/s, %.1f s\n",
                          ...
                          unsigned *actual_workers)
{
    ...
    *actual_workers = workers;
    ...
    if (report) {
        fprintf(stderr,
                "398903_04: n=%d, Gray-code vertex IE, chunks=%" PRIu64
...
static void refuse_bfile_shrink(const char *path, int requested_maximum)
{
    ...
    while (fgets(line, sizeof(line), input) != NULL) {
        if (strchr(line, '\n') == NULL && !feof(input)) {
            die("existing b-file has an overlong line");
        }
        int index = -1;
        char value_text[40];
        char extra;
        U128 ignored;
        if (sscanf(line, "%d %39s %c", &index, value_text, &extra) != 2 ||
            index != expected || index < MIN_N || index > MAX_N ||
            !parse_decimal_u128(value_text, &ignored)) {
            die("existing b-file is not a valid consecutive prefix");
        }
        ++expected;
    }
    if (ferror(input)) die("cannot read existing b-file");
    if (fclose(input) != 0) die("cannot close existing b-file");

    const int existing_maximum = expected - 1;
    if (existing_maximum > requested_maximum) {
        fprintf(stderr,
                "error: existing b-file ends at n=%d; refusing to replace "
                "it by the shorter prefix n=0..%d\n",
                existing_maximum, requested_maximum);
        exit(EXIT_FAILURE);
    }
}
...
{
    char *final_path = path_beside_executable(argv0, "b398903_04.txt");
    char *part_path = path_beside_executable(argv0, "b398903_04_part.txt");
    char *lock_path = path_beside_executable(argv0, "b398903_04.lock");
    const int lock_fd = lock_bfile(lock_path);
    refuse_bfile_shrink(final_path, maximum_n);
    ...
    fprintf(stderr, "398903_04: wrote actually computed terms n=0..%d to %s\n",

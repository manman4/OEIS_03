/*
 * A398903 -- deliberately naive exact counter for partitions of {1,...,3*n} into
 * unordered three-element subsets {x,y,z} satisfying x+y<=z.
 *
 * For positive distinct elements, x+y<=z forces z to be the largest
 * element.  Therefore generate_rows() writes a<b<c and accepts the set
 * exactly when a+b<=c.  Each valid unordered three-set is generated once.
 *
 ...
 *       398903_01.c -o 398903_01
 *
 * Usage:
 *   ./398903_01 5
 *   ./398903_01 --term 5
 *   ./398903_01 --check --no-bfile
 *
 * A positional N computes a(0),...,a(N).  Completed terms are atomically
 * recorded in b398903_01.txt by default.  --term N requires a complete
...
#error "398903_01.c requires unsigned __int128"
#endif
...
static const char *output_path = "b398903_01.txt";
...
static void record_term(int n, U128 value)
{
    ...
    if (failed) die("cannot write temporary b-file");
    if (rename(temporary, output_path) != 0) {
        die("cannot install temporary b-file");
    }
    free(temporary);
    unlock_bfile(lock_fd);

    fprintf(stderr, "398903_01: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static void heartbeat_if_due(void)
{
    ...
    fprintf(stderr, "398903_01: heartbeat n=%d, calls=", active_n);
    print_calls(stderr);
    fprintf(stderr, ", %.1f s\n", now - started);
...
static U128 compute_term(int n)
{
    ...
    fprintf(stderr,
            "398903_01: n=%d, naive backtracking, rows=%" PRIu32
...
{
    static const uint64_t expected[] = {1, 1, 3, 28, 487, 14077};
    for (int n = 0; n <= 5; ++n) {
    ...
    fprintf(stderr, "398903_01: self-check passed for n=0..5\n");
}
...
static void usage(FILE *stream, const char *program)
{
    fprintf(stream,
            ...
            "  --output FILE use FILE instead of b398903_01.txt\n"

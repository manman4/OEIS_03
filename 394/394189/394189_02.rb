...
def count_partitions_with_constraint_lt_up_to(n_max)
  # count partitions where (distinct parts) < minimal multiplicity
  ...
  loop do
    ...
    # min multiplicity >= k + 1 for exactly k distinct parts
    at_least_k1 = count_exact_d_distinct_with_min_mult_up_to(n_max, k, k + 1)

    (0..n_max).each{|s|
      counts[s] += at_least_k1[s]
    }
    k += 1
  end
...
n = 1000
counts = count_partitions_with_constraint_lt_up_to(n)

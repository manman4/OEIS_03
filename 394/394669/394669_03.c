...
typedef struct {
    ...
    int32_t *nz_pos;   // index -> position in nz list, -1 if zero
    size_t *nz;        // list of indices with nonzero coeffs
    size_t nz_count;
    size_t nz_cap;
    ...
    size_t nonzero_cnt; // cached count of nonzero coeffs (kept in sync with nz_count)
} Poly;
...
static void poly_init(Poly *p, size_t cap_table, size_t cap_terms) {
    ...
    p->nz_pos = (int32_t *)malloc(sizeof(int32_t) * cap_terms);
    if (!p->nz_pos) die_oom();
    for (size_t i = 0; i < cap_terms; i++) p->nz_pos[i] = -1;

    p->nz = (size_t *)malloc(sizeof(size_t) * cap_terms);
    if (!p->nz) die_oom();
    p->nz_count = 0;
    p->nz_cap = cap_terms;
...
static void poly_clear(Poly *p) {
    ...
    for (size_t i = 0; i < old_size; i++) p->nz_pos[i] = -1;
    p->nz_count = 0;
    p->max_idx = (size_t)SIZE_MAX;
    p->max_dirty = 0;
    mpz_set_ui(p->max_abs, 0);
...
static void poly_reserve_terms(Poly *p, size_t new_cap) {
    ...
    int32_t *np = (int32_t *)realloc(p->nz_pos, sizeof(int32_t) * new_cap);
    if (!np) die_oom();
    if (new_cap > p->cap_terms) {
        for (size_t i = p->cap_terms; i < new_cap; i++) np[i] = -1;
    }
    p->nz_pos = np;

    if (new_cap > p->nz_cap) {
        size_t *nz2 = (size_t *)realloc(p->nz, sizeof(size_t) * new_cap);
        if (!nz2) die_oom();
        p->nz = nz2;
        p->nz_cap = new_cap;
    }
...
static inline void poly_add_term_mpz(Poly *p, Term t, const mpz_t add) {
    ...
    while (1) {
        ...
        if (idx == 0xFFFFFFFFu) {
            ...
            p->size++;
            p->nonzero_cnt++;
            p->nz_pos[pos] = (int32_t)p->nz_count;
            p->nz[p->nz_count++] = pos;
        ...
        if (term_eq(p->terms[idx], t)) {
            ...
            if (before == 0 && after != 0) {
                p->nonzero_cnt++;
                p->nz_pos[idx] = (int32_t)p->nz_count;
                p->nz[p->nz_count++] = idx;
            }
            if (before != 0 && after == 0) {
                if (p->nonzero_cnt > 0) p->nonzero_cnt--;
                int32_t pos = p->nz_pos[idx];
                if (pos >= 0) {
                    size_t last = p->nz[--p->nz_count];
                    p->nz[pos] = last;
                    p->nz_pos[last] = pos;
                    p->nz_pos[idx] = -1;
                }
...
static void poly_compact(Poly *dst, const Poly *src) {
    poly_clear(dst);
    for (size_t k = 0; k < src->nz_count; k++) {
        size_t i = src->nz[k];
...
static void poly_div_var_scale_neg_ui(Poly *dst, const Poly *src, int var, unsigned long k) {
    mpz_t tmp;
    mpz_init(tmp);
    for (size_t idx = 0; idx < src->nz_count; idx++) {
        size_t i = src->nz[idx];
        Term term = src->terms[i];
        if (!dec_var(&term, var)) continue;

        mpz_divexact_ui(tmp, src->coeffs[i], k);
        mpz_neg(tmp, tmp);
        poly_add_term_mpz(dst, term, tmp);
    }
    mpz_clear(tmp);
}
...
static void term_print_monomial(FILE *out, Term t, int max_var) {
    int first = 1;
    for (int v = 1; v <= max_var; v++) {
        uint8_t e = get_exp(t, v);
        if (e == 0) continue;
        if (!first) fprintf(out, " ");
        if (e == 1) {
            fprintf(out, "a%d", v);
        } else {
            fprintf(out, "a%d^%u", v, (unsigned)e);
        }
        first = 0;
    }
    if (first) fprintf(out, "1");
}
...
static Term poly_max_term(Poly *p) {
    if (p->size == 0) {
        return term_zero();
    }
    if (p->max_dirty || p->max_idx == (size_t)SIZE_MAX) {
    ...
    if (p->max_idx == (size_t)SIZE_MAX) return term_zero();
    return p->terms[p->max_idx];
}

static void poly_print_all_max_terms(FILE *out, Poly *p, int max_var) {
    if (p->size == 0) {
    ...
    for (size_t k = 0; k < p->nz_count; k++) {
        ...
        if (mpz_cmpabs(p->coeffs[i], p->max_abs) == 0) {
            max_cnt++;
            int sgn = mpz_sgn(p->coeffs[i]);
            fprintf(out, "  sign=%c  ", (sgn < 0 ? '-' : '+'));
            term_print_monomial(out, p->terms[i], max_var);
            fprintf(out, "\n");
        }
    }
    fprintf(out, "maxterms count: %zu\n", max_cnt);
    // second pass: exponent vector format
    fprintf(out, "maxterms (exp vectors):\n");
    for (size_t k = 0; k < p->nz_count; k++) {
        size_t i = p->nz[k];
        if (mpz_cmpabs(p->coeffs[i], p->max_abs) == 0) {
            int sgn = mpz_sgn(p->coeffs[i]);
            ...
            for (int v = 1; v <= max_var; v++) {
                if (v > 1) fprintf(out, ", ");
                fprintf(out, "%u", (unsigned)get_exp(p->terms[i], v));
            }
            fprintf(out, "]\n");
        }
    }
...
static void print_monic_poly(FILE *out, int n) {
    fprintf(out, "f(x)=x^%d", n);
    for (int i = 1; i <= n; i++) {
        int pow = n - i;
        if (pow == 1) {
            fprintf(out, " + a%d*x", i);
...
int main(int argc, char **argv) {
    ...
    for (int n = 3; n <= max_n; n++) {
        ...
        // s = D0
        poly_clear(&s);
        for (size_t k = 0; k < t0.nz_count; k++) {
            size_t i = t0.nz[k];

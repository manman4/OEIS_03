/*
 * A374288: least nonnegative k having exactly n nonnegative
 * integer solutions (x,y) satisfying
 *
 *                         x^2 + 5 y^2 = k.
 *
 * This is an exact prime-exponent search, not a scan up to a guessed bound.
 * The form has fundamental discriminant -20 and class group of order two.
 * For a represented positive integer k, write
 *
 *   D(k) = product (e_p+1),
 *
 * over split primes p. The principal split primes are 1 or 9 (mod 20),
 * and the nonprincipal split primes are 3 or 7 (mod 20). Primes congruent
 * to 11,13,17,19 (mod 20) are inert and must have even exponent. If s is
 * the exponent of 2, representation by the principal form requires
 *
 *   s + sum e_q == 0 (mod 2),  q == 3 or 7 (mod 20).
 *
 * The ramified prime 5 is principal and does not enter this parity condition.
 * Let A(k)=1 when k is a square or five times a square, and A(k)=0
 ...
 * In addition, the proved theorem
 *
 *                         9 | a(n) for n > 0
 *
 * is used as an exact pruning rule. For n>=2, only assignments in which
 * the nonprincipal split prime 3 has exponent at least two are retained.
 * The initial values a(0)=2 and a(1)=0 are handled separately, so the
 * theorem guarantees that the true minimum is never discarded.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374288_02.c \
 *       -o 374288_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374288_02 --target 23 --verbose
 *   ./374288_02 --upto 100
 *   ./374288_02 --self-test 1000000
 */
...
#define BFILE_NAME "b374288_02.txt"
#define BFILE_TEMP_NAME "b374288_02.txt.tmp"
                                ...
                                unsigned nonprincipal_parity)
{
    ...
    /* nonprincipal_primes[0]=3; reject before making GMP products. */
    if (assignment->search->require_multiple_of_nine &&
        (assignment->nonprincipal_count == 0 ||
         assignment->nonprincipal_exponents[0] < 2))
        return;
...
static uint64_t representation_count_q5(uint64_t k)
{
    if (k == 0) return 1;
    ...
    if (remaining > 1) {
        const uint64_t residue = remaining % 20;
        if (residue == 1 || residue == 9) {
            d *= 2;
        } else if (residue == 3 || residue == 7) {
            d *= 2;
    ...
    const uint64_t axis =
        is_square_u64(k) || (k % 5 == 0 && is_square_u64(k / 5));
    return (d + axis) / 2;
}
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 0; 5 * y * y <= limit; ++y) {
        const uint64_t y_term = 5 * y * y;
    ...
    for (uint64_t k = 0; k <= limit; ++k) {
        const uint64_t got = representation_count_q5(k);
        if (got != direct[k]) {
    ...
    static const Regression known_regressions[] = {
        {0, 2}, {1, 0}, {2, 9}, {3, 81}, {4, 189}, {5, 441},
        {6, 1449}, {7, 10206}, {8, 3969}, {9, 12789}, {10, 13041},
        {11, 35721}, {12, 30429}, {13, 194481}, {14, 117369},
        {15, 115101}, {16, 186921}, {17, 2893401}, {18, 273861},
        {20, 378189}, {21, 1035909}, {22, 9506889}, {23, 2099601},
        {24, 882441}, {25, 5639949}, {27, 6765381}, {28, 3403701},
        {30, 3925341}, {32, 5420709}, {35, 50759541}, {36, 7941969},
        {40, 15505749}, {42, 35328069}, {48, 36180081}
    };
    for (size_t i = 0;
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (n == 0 || n == 1) {
            ...
            for (unsigned i = 0; i < answer.prime_count; ++i) {
                ...
                const uint64_t residue = answer.primes[i] % 20;
                if (residue == 3 || residue == 7)
                    nonprincipal_parity ^= answer.exponents[i] & 1;
                else if (residue != 1 && residue != 9)
                    passed = false;
        ...
        if (passed && mpz_fits_ulong_p(answer.value)) {
            ...
            passed = (uint64_t)value == value &&
                     representation_count_q5((uint64_t)value) == n;

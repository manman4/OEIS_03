/*
 * A374285: least k having exactly n nonnegative solutions of
 *
 *                    x^2 + 2 y^2 = k.
 *
 ...
 *
 * Put D(k) = product (e_p+1), over primes p == 1 or 3 (mod 8).  If k is
 * representable, its number R(k) of solutions with x,y >= 0 is
 *
 *     R(k) = (D(k) + A(k))/2,
 *
 * where A(k)=1 if k is a square or twice a square, and A(k)=0 otherwise.
 * An odd exponent at a prime p == 5 or 7 (mod 8) gives no representations.
 *
 * Hence R(k)=n permits only D=2n when A=0 or D=2n-1 when A=1.
 ...
 * all k below the answer.  The exceptional initial values are a(0)=5 and
 * a(1)=0.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374285_01.c \
 *       -o 374285_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374285_01 --target 23 --verbose
 *   ./374285_01 --upto 100
 *   ./374285_01 --self-test 1000000
 */
...
#define BFILE_NAME "b374285.txt"
#define BFILE_TEMP_NAME "b374285.txt.tmp"
...
static void calculate_answer(uint64_t n, Answer *answer)
{
    ...
    if (n == 0) {
        search.best.found = true;
        mpz_set_ui(search.best.value, 5);
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 0; 2 * y * y <= limit; ++y)
        for (uint64_t x = 0; x * x <= limit - 2 * y * y; ++x)
            ++direct[x * x + 2 * y * y];
    ...
    static const Regression oeis_regressions[] = {
        {0, 5}, {1, 0}, {2, 9}, {3, 81}, {4, 297}, {5, 891},
        {6, 1683}, {7, 8019}, {8, 5049}, {9, 18513}, {10, 15147},
        {11, 88209}, {12, 31977}, {13, 1185921}, {14, 136323},
        {15, 166617}, {16, 95931}, {17, 7144929}, {18, 351747},
                         ...
                         const mpz_t limit, bool verbose)
{
    ...
    if (verbose && n != 0) {
        if (n == 1) {
            printf("special initial case: k=0 has the solution (0,0)\n");
            return;
        }
        uint64_t primes[MAX_FACTORS];

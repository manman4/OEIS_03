...
#       R(k) = 2*M   ... v = 0
#            = 6*M   ... v >= 2 かつ v が偶数
#            = 0     ... v が奇数（k ≡ 2 mod 4 だけでなく k = 8, 24, 32, ... も）
#   2 は Q(sqrt(-3)) で惰性なので、v が奇なら R = 0。v が偶で正なら
#   単数群の寄与が 3 倍になる、というのがこの補正の正体。
...
#       v >= 2 かつ偶  -> P = (3M - eps) / 2
#       v = 0          -> P = (M - eps) / 2
#   （v が偶であることが R != 0 の必要条件なので、4 | k の側で最安なのは v = 2）
#
#   eps は M のパリティで自動的に決まる:
#     M が奇 <=> 全 e_i が偶 <=> p 部分が平方
#       v = 0 : k = 3^c * (平方) なので c の偶奇で必ず eps = 1
#       v 偶  : k = 2^v * 3^c * (平方) でやはり eps = 1

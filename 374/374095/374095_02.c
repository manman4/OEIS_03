/*
 * A374095, version 02: least nonnegative k having exactly n solutions of
 *
 *                    x^2 + 3*x*y + y^2 = k,   0 < x < y.
 *
 * This is an exact prime-exponent search, not a scan with a guessed cutoff.
 * For a representable positive k, put
 *
 *     D(k) = product (e_p + 1),
 *
 ...
 * norm-one units identify generators in a fundamental cone.  This gives
 * D(k)-S(k) positive ordered pairs, where S(k)=1 exactly when k is a square.
 * The remaining diagonal solution x=y exists exactly when k is five times
 * a square.  Hence
 *
 *     R_<(k) = (D(k) - A(k))/2,
 *
 ...
 * Therefore R_<(k)=n permits only D=2*n with A=0, or D=2*n+1 with A=1.
 *
 * A multiplicative partition D=product(e_i+1) lists every exponent
 * multiset.  Its least integer assigns the largest e_i to 11, the next to 19,
 * then 29, 31, ... (the primes 1 or 4 mod 5).  A=1 is equivalent to all e_i
 * being even; A=0 is equivalent to at least one e_i being odd.  Powers of 5 and
 * squares of inert primes only increase the candidate, so none is needed.
 * This proves that the finite exponent-pattern search returns the true
 * minimum.  The exceptional initial value is a(0)=0.
 *
 * This version additionally uses the proved theorem
 *
 *                         11 | a(n).
 *
 * For every n>=1, candidates are constructed explicitly as 11*u: one copy
 * of the least split prime 11 is installed first, and only the remaining
 * exponent is raised during candidate construction.  Thus every positive
 * candidate satisfies the theorem by construction.  The self-test also runs
 * the unrestricted version-01 construction and checks that both searches
 * agree through n=1000.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374095_02.c \
 *       -o 374095_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374095_02 --target 10 --verbose
 *   ./374095_02 --upto 100
 *   ./374095_02 --self-test 1000000
 */
...
#define BFILE_NAME "b374095_02.txt"
#define BFILE_TEMP_NAME "b374095_02.txt.tmp"
...
static void consider_partition(Search *search, uint64_t d, bool axis)
{
    ...
    if (search->factor_count == 0)
        die("positive D must have a nonempty exponent pattern");

    /* By 11 | a(n), construct optimized candidates explicitly as 11*u. */
    const uint64_t first_exponent =
        search->factors[search->factor_count - 1] - 1;
    if (search->use_divisibility_theorem && first_exponent == 0) return;

    mpz_t value, power;
    mpz_init_set_ui(value, search->use_divisibility_theorem ? 11U : 1U);
    mpz_init(power);
    for (unsigned i = 0; i < search->factor_count; ++i) {
        const uint64_t exponent =
            search->factors[search->factor_count - 1 - i] - 1;
        const uint64_t exponent_in_cofactor = exponent -
            (search->use_divisibility_theorem && i == 0 ? 1U : 0U);
                                      ...
                                      bool use_divisibility_theorem)
{
    ...
    search.use_divisibility_theorem = use_divisibility_theorem;
    if (n == 0) {
        search.best.found = true;
    ...
    search_case(&search, 2 * n, false);
    search_case(&search, 2 * n + 1, true);
    answer_copy(answer, &search.best);
    answer_clear(&search.best);
}
...
static unsigned representation_count_formula(uint64_t k)
{
    ...
    const unsigned axis = is_square_u64(k) ||
        (k % 5 == 0 && is_square_u64(k / 5));
    return (unsigned)((d - axis) / 2);
}
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 2; y * y + 3 * y + 1 <= limit; ++y) {
        const uint64_t y_square = y * y;
        for (uint64_t x = 1;
             x < y && x * x + 3 * x * y <= limit - y_square; ++x)
    ...
    static const Regression oeis_regressions[] = {
        {0, 0}, {1, 11}, {2, 209}, {3, 2299}, {4, 6061},
        {5, 278179}, {6, 66671}, {7, 5285401}, {8, 187891},
        {9, 1266749}, {10, 8067191}
    };
    for (size_t i = 0;
        ...
        const bool passed = answer.found &&
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        bool passed = answer.found && unrestricted.found &&
        ...
        if (n == 0) {
            ...
            passed = passed && mpz_divisible_ui_p(answer.value, 11) != 0 &&
                     reconstructed_d == answer.d &&
                     all_even == answer.axis &&
                     (reconstructed_d - (answer.axis ? 1 : 0)) / 2 == n &&
    ...
    printf("self-test passed: direct enumeration agrees for 0 <= k <= %"
           ...
           "agree, the optimized and unrestricted searches match, the "
           "11-divisibility theorem holds, and exact structural checks pass "
           "through n=%" PRIu64 "\n",
    ...
    if (have_limit) {
        if (mpz_cmp(answer->value, limit) > 0) {
            ...
            printf(" with exactly %" PRIu64
                   " representations satisfying 0 < x < y\n", n);
        } else {
...
static void usage(const char *program)
{
    fprintf(stderr,
        ...
        "Exact structural output is tested through n=%" PRIu64 ".\n"

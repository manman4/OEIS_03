/*
 * A374286: least k having exactly n nonnegative solutions of
 *
 *                    x^2 + 3 y^2 = k.
 *
 ...
 * is representable, the number R(k) of solutions with x,y >= 0 is
 *
 *     R(k) = (C D(k) + A(k))/2,
 *
 * where C=1 if v_2(k)=0, C=3 if positive v_2(k) is even, and A=1 if k is a
 * square or three times a square (otherwise A=0).  Odd v_2(k), or an odd
 * exponent at an odd prime p == 2 (mod 3), gives no representations.
 *
 * Hence R(k)=n permits only D=2n-A for C=1 or D=(2n-A)/3 for C=3.
 ...
 * iterating through all k below the answer.  The exceptional initial values
 * are a(0)=2 and a(1)=0.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374286_01.c \
 *       -o 374286_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374286_01 --target 23 --limit 2^40 --verbose
 *   ./374286_01 --upto 23
 *   ./374286_01 --self-test 1000000
 */
...
#define BFILE_NAME "b374286.txt"
#define BFILE_TEMP_NAME "b374286.txt.tmp"
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 0; 3 * y * y <= limit; ++y)
        for (uint64_t x = 0; x * x <= limit - 3 * y * y; ++x)
            ++direct[x * x + 3 * y * y];
    ...
    /* Values recorded in the A374286 data and comments. */
    static const Regression oeis_regressions[] = {
        {0, 2, false}, {1, 0, false}, {2, 4, false},
        {3, 28, false}, {4, 1729, false}, {5, 196, false},
        {6, 364, false}, {7, 1529437, false}, {8, 9604, false},
        {9, 2548, false}, {10, 593047, false}, {11, 470596, false},
        {12, 6916, false}, {13, 68574961, false}, {14, 33124, false},
        {15, 124852, false}, {16, 1983163, false},
        {17, 1129900996, false}, {18, 48412, false},
        {20, 18384457, false}, {21, 6117748, false},
        {23, 1623076, false}, {24, 214396, false},
        {27, 629356, false}, {30, 2372188, false},
        {32, 79530724, false}, {36, 1500772, false},
        {41, 11957764, false}, {42, 116237212, false}
    };
    for (size_t i = 0;
                         ...
                         const mpz_t limit, bool verbose)
{
    ...
    if (verbose && n != 0) {
        ...
        printf("case: C=%u, A=%u, D=%" PRIu64 "; factorization: ",

/*
 * A374090: least nonnegative k having exactly n ordered positive solutions of
 *
 *                    x^2 + x*y + y^2 = k.
 *
 ...
 *
 * Put D(k) = product (e_p+1), over primes p == 1 (mod 3).  If k is
 ...
 * at a prime p == 2 (mod 3) gives no representations.  The ramified prime
 * 3 does not affect D(k).
 *
 ...
 * multiset.  Its smallest represented part assigns the largest e_i to 7,
 * the next to 13, then 19, 31, etc.  If all e_i are even but S=0 is required,
 * multiplying by 3 gives the least nonsquare without changing D.  Squares of
 * inert primes
 * only increase k.  Thus the search proves minimality without iterating
 * through all k below the answer.  The exceptional initial value is a(0)=0.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374090_01.c \
 *       -o 374090_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374090_01 --target 23 --verbose
 *   ./374090_01 --upto 100
 *   ./374090_01 --self-test 1000000
 */
...
#define BFILE_NAME "b374090.txt"
#define BFILE_TEMP_NAME "b374090.txt.tmp"
...
static void answer_copy(Answer *destination, const Answer *source)
{
    ...
    destination->factor_three = source->factor_three;
...
static void consider_partition(Search *search, uint64_t d, bool axis)
{
    ...
    mpz_t value, power;
    const bool factor_three = !axis && all_even;
    mpz_init_set_ui(value, factor_three ? 3 : 1);
    mpz_init(power);
    for (unsigned i = 0; i < search->factor_count; ++i) {
        const uint64_t exponent =
    ...
    if (!search->best.found || mpz_cmp(value, search->best.value) < 0) {
        ...
        search->best.factor_three = factor_three;
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 1; y * y + y + 1 <= limit; ++y) {
        const uint64_t y_square = y * y;
        for (uint64_t x = 1;
             x * x + x * y <= limit - y_square; ++x)
    ...
    static const Regression oeis_regressions[] = {
        {0, 0}, {1, 3}, {2, 7}, {3, 147}, {4, 91}, {5, 7203},
        {6, 637}, {7, 352947}, {8, 1729}, {9, 24843}, {10, 31213},
        {11, 847425747}, {12, 12103}, {13, 41523861603},
        {14, 405769}, {15, 1217307}, {16, 53599},
        {17, 99698791708803}, {18, 157339},
        {19, 4885240793731347}, {20, 593047}, {21, 59648043},
        {24, 375193}, {25, 205724883}, {26, 2989441},
        {27, 8968323}, {28, 29059303}, {30, 7709611},
        {32, 1983163}, {33, 143214951243}, {35, 10080519267},
        {36, 4877509}, {45, 439447827}, {49, 1703607756123},
        {63, 21532943523}, {75, 74266682763}, {81, 8618558403},
        {135, 422309361747}
    };
    for (size_t i = 0;
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (n == 0) {
            ...
            mpz_init_set_ui(reconstructed, answer.factor_three ? 3U : 1U);
            mpz_init(power);
            for (unsigned i = 0; i < answer.exponent_count; ++i) {
            ...
            passed = passed && reconstructed_d == answer.d &&
                     ...
                     answer.factor_three == (!answer.axis && all_even) &&
                     mpz_cmp(reconstructed, answer.value) == 0;
            if ((n & 1) != 0)
                passed = passed && !answer.axis && answer.factor_three;
            mpz_clear(power);

/*
 * A374094: least nonnegative k having exactly n solutions of
 *
 *                    x^2 + x*y + y^2 = k,   0 < x < y.
 *
 ...
 *     D(k) = product (e_p + 1),
 *
 * over primes p == 1 (mod 3), where e_p is the exponent of p in k.
 * Primes q == 2 (mod 3) must have even exponent.  The full integer
 * representation formula for the Eisenstein norm gives 6*D(k) solutions.
 * Restricting to positive ordered pairs, removing the axes, gives D(k)-S(k),
 * where S(k)=1 exactly when k is a square.  The remaining diagonal solution
 * x=y exists exactly when k is three times a square.  Hence
 *
 *     R_<(k) = (D(k) - A(k))/2,
 *
 * where A(k)=1 when k is a square or three times a square, and A(k)=0
 ...
 * multiset.  Its least integer assigns the largest e_i to 7, the next to 13,
 * then 19, 31, ... (the primes 1 mod 3).  A=1 is equivalent to all e_i being
 * even; A=0 is equivalent to at least one e_i being odd.  Powers of 3 and
 ...
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 374094_01.c \
 *       -o 374094_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./374094_01 --target 10 --verbose
 *   ./374094_01 --upto 100
 *   ./374094_01 --self-test 1000000
 */
...
#define BFILE_NAME "b374094.txt"
#define BFILE_TEMP_NAME "b374094.txt.tmp"
...
static unsigned representation_count_formula(uint64_t k)
{
    ...
    while (remaining % 3 == 0) remaining /= 3;
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 2; y * y + y + 1 <= limit; ++y) {
        const uint64_t y_square = y * y;
        for (uint64_t x = 1;
             x < y && x * x + x * y <= limit - y_square; ++x)
            ++direct[x * x + x * y + y_square];
    }
    ...
    static const Regression oeis_regressions[] = {
        {0, 0}, {1, 7}, {2, 91}, {3, 637}, {4, 1729},
        {5, 31213}, {6, 12103}, {7, 405769}, {8, 53599},
        {9, 157339}, {10, 593047}, {12, 375193},
        {13, 2989441}, {14, 29059303}, {15, 7709611},
        {16, 1983163}, {18, 4877509}
    };
    for (size_t i = 0;

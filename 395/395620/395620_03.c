/*
 * A395620 -- bidirectional middle-frontier DP with exact CRT recovery.
 *
 ...
 *
 * This is deliberately different from 395620_02.c: 02 propagates one DP
 ...
 *
 * One uint8_t residue is stored per state.  Seventeen passes through
 * pairwise distinct primes below 256 suffice at n=34 because their product
 * exceeds 34!.  Incremental CRT in a checked 256-bit integer therefore
 * recovers the unique exact answer.  The largest heap-plus-mapped-frontier
 * footprint at n=34 is 4,537,567,650 bytes (about 4.226 GiB); the temporary
 * file is 2,333,606,220 bytes (about 2.174 GiB).  Runtime remains O(n*2^n).
 *
 * Cardinality layers are divided into colex-rank ranges and evaluated by
 * pthread workers.  A395620_THREADS controls the count (default: online CPU
 * count capped at 8).  A395620_MEMORY_MIB controls the checked memory budget
 * (default: 3072 MiB).
 *
 * Known terms a(0)..a(27) are built in.  Every newly completed term is saved
 * atomically in b395620_03.txt, allowing interrupted --upto runs to resume.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       395620_03.c -o 395620_03
 *
 * Examples:
 *   ./395620_03 --term 31
 *   ./395620_03 --upto 33
 *   A395620_MEMORY_MIB=5000 ./395620_03 --term 34
 *   ./395620_03 --check
 */
...
#error "395620_03.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "395620_03.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b395620_03.txt"
#define LOCK_PATH "b395620_03.txt.lock"
...
{
    char path[] = "395620_03.frontier.XXXXXX";
                         ...
                         uint64_t count, double seconds)
{
    if (!quiet && count >= REPORT_MIN_STATES) {
        fprintf(stderr,
                "395620_03: n=%u pass=%u/%u %s-layer=%u states=%" PRIu64
                " time=%.1fs\n",
                                    ...
                                    unsigned pass, unsigned pass_count)
{
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_03: n=%u CRT pass %u/%u p=%" PRIu32 "\n",
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_03: n=%u pass=%u/%u middle join time=%.1fs\n",
                n, pass + 1U, pass_count,
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_03: n=%u CRT pass %u/%u done time=%.3fs\n",
                n, pass + 1U, pass_count,
...
static U256 count_exact(unsigned n)
{
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_03: n=%u peak=%.3f GiB temp=%.3f GiB "
    ...
    if (!quiet) {
        fprintf(stderr, "395620_03: n=%u exact answer time=%.3fs\n",
...
static void record_term(unsigned n, const U256 *value)
{
    ...
    char temporary[] = "b395620_03.txt.tmp.XXXXXX";
    ...
    unlock_bfile(lock_descriptor);
    if (!quiet) {
        fprintf(stderr, "395620_03: saved a(%u) in %s\n", n, BFILE_PATH);
    }
}
...
static U256 obtain_term(unsigned n)
{
    ...
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "395620_03: reusing saved a(%u)\n", n);
        }
        return value;
    }
...
static void usage(const char *program, FILE *stream)
{
    fprintf(stream,
            ...
            "A395620_MEMORY_MIB sets the memory limit (default %" PRIu64
            " MiB).\n"

/*
 * A395620 -- low-memory modular layer DP with exact CRT recovery.
 *
 * Count permutations s_1,...,s_n of [n] for which no nonempty prefix sum
 * is a perfect square.  For S subseteq [n], let F(S) count valid orders
 * of S.  Then
 *
 *   F(empty) = 1,
 *   F(S) = 0                   if sum(S) is a perfect square,
 *   F(S) = Sum_{x in S} F(S \ {x})       otherwise.
 *
 * As in 395620_01.c, fixed-cardinality subsets are enumerated in colex
 ...
 * atomically in b395620_02.txt, so a later run can reuse it.
 *
 * Layers are divided into contiguous colex-rank ranges and evaluated in
 * parallel.  A395620_THREADS controls the worker count; the default is the
 * number of online processors, capped at 8.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       395620_02.c -o 395620_02
 *
 * Examples:
 *   ./395620_02 --term 31
 *   ./395620_02 --upto 32
 *   ./395620_02 --check
 *   A395620_MEMORY_MIB=5000 ./395620_02 --term 34
 */
...
#error "395620_02.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "395620_02.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b395620_02.txt"
#define LOCK_PATH "b395620_02.txt.lock"
...
static const char *const known_terms[KNOWN_MAX_N + 1U] = {
    ...
    "34553805131115392",
    "670580814038650976",
    "14155521644833590976",
    "323811356025442369136",
    "7129269277696485742416",
    "172625645396753955332960",
    "4091834434892742591232864",
    "109483439018513119215194624",
    "2903413384312431737278567648"
};
...
static void configure_memory_limit(void)
{
    ...
    const char *text = getenv("A395620_MEMORY_MIB");
    if (text != NULL && *text != '\0') {
        char *end = NULL;
        errno = 0;
...
static void configure_threads(void)
{
    ...
    const char *text = getenv("A395620_THREADS");
    if (text != NULL && *text != '\0') {
        threads = parse_unsigned(text, MAX_THREADS, "A395620_THREADS");
        if (threads == 0U) {
                             ...
                             unsigned pass_count)
{
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_02: n=%u CRT pass %u/%u p=%" PRIu32
                " cell=%u byte%s\n",
    ...
    for (unsigned cardinality = 1U; cardinality <= n; ++cardinality) {
        ...
        if (report) {
            fprintf(stderr,
                    "395620_02: n=%u pass=%u/%u layer=%u states=%" PRIu64
                    " started\n",
        ...
        if (report) {
            fprintf(stderr,
                    "395620_02: n=%u pass=%u/%u layer=%u done time=%.1fs\n",
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_02: n=%u CRT pass %u/%u done time=%.3fs\n",
...
static U256 count_exact(unsigned n)
{
    ...
    if (!quiet) {
        fprintf(stderr,
                "395620_02: n=%u peak=%.3f GiB CRT_passes=%u threads=%u "
                "bound=n!\n",
                n, (double)peak / (1024.0 * 1024.0 * 1024.0),
    ...
    if (!quiet) {
        fprintf(stderr, "395620_02: n=%u exact answer time=%.3fs\n",
...
static void record_term(unsigned n, const U256 *value)
{
    ...
    char temporary[] = "b395620_02.txt.tmp.XXXXXX";
    ...
    unlock_bfile(lock_descriptor);
    if (!quiet) {
        fprintf(stderr, "395620_02: saved a(%u) in %s\n", n, BFILE_PATH);
    }
}
...
static U256 obtain_term(unsigned n)
{
    ...
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "395620_02: reusing saved a(%u)\n", n);
        }
        return value;
    }
...
static void usage(const char *program, FILE *stream)
{
    fprintf(stream,
            ...
            " MiB).\n"
            "A395620_THREADS sets 1..%u worker threads (default: up to 8).\n",

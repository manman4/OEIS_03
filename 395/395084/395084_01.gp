...
my(N=15, x='x+O('x^N)); print(sum(k=1, N, a(k)/k * (x/(1+(k+3)^2*x))^k) - log(1+9*x));

print();

\\ 1 = Sum_{k>=0} a(k) * binomial(k+m-1,k) * x^k/(1 + (k+3)^2*x)^(k+m) for m >= 1.
for(m=1, 5, my(N=15, x='x+O('x^N)); print(sum(k=0, N, a(k) * binomial(k+m-1,k) * x^k/(1+(k+3)^2*x)^(k+m))));

print();

\\ 1 = Sum_{k>=0} a(k) * x^k/k! * exp(-(k+3)^2*x).
my(N=15, x='x+O('x^N)); print(sum(k=0, N, a(k) * x^k/k! * exp(-(k+3)^2*x)));

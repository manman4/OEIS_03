...
c(n) = sum(k=0, n, binomial(5*n+2*k-3,k));
for(n=0, 50, print1(a(5,2,0,1,0,n)-c(n),", "));

\\ a(n) = Sum_{k=0..n} (-1)^k * binomial(7*n+k-1,n-k).
d(n) = sum(k=0, n, (-1)^k * binomial(7*n+k-1,n-k));
for(n=0, 50, print1(a(5,2,0,1,0,n)-d(n),", "));

\\ a(n) = Sum_{k=0..floor(n/2)} (-1)^k * binomial(7*n-k-2,n-2*k).
e(n) = sum(k=0, n\2, (-1)^k * binomial(7*n-k-2,n-2*k));
for(n=0, 50, print1(a(5,2,0,1,0,n)-e(n),", "));
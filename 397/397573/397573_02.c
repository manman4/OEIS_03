/*
 * A397573 -- fast deterministic exact-cover counter.
 *
 * Count partitions of {1,...,4*n} into unordered four-element blocks that
 * can be split into two pairs with the same sum.  Write a block as
 * a < b < c < d.  Two of its pairings are strictly unequal,
 *
 *     a+b < c+d,             a+c < b+d,
 *
 * so the block is valid exactly when a+d = b+c.  This also proves that the
 * equal-sum split is unique.  generate_rows() lists every valid four-set once
 * by choosing a<b<c and setting d=b+c-a.
 *
 * The rows form an exact-cover instance whose columns are the values 1..4*n.
 * At a remaining-value mask M, minimum_column() chooses a value contained in
 * the fewest rows wholly inside M.  Exactly one row of every completion covers
 * that value.  Trying all such rows and recursing on M\row therefore counts
 * every unordered partition once, by induction on popcount(M)/4.  A column
 * with no compatible row is an exact dead end, not a heuristic cutoff.
 *
 * A subproblem is completely determined by M.  Each worker has a bounded
 * direct-mapped cache from the full 64-bit mask to its exact U128 count.  Both
 * the key and value are checked on lookup.  A hash collision only replaces an
 * older entry and causes possible recomputation; it cannot produce a false
 * result.  The cache is consequently an optimization, not a probabilistic
 * assumption.  Workers have private caches, so the parallel search is free of
 * cache data races.
 *
 * Reflection i -> 4*n+1-i preserves a+d=b+c and bijects exact covers.  Masks
 * related by reflection therefore have equal counts, so the cache uses their
 * smaller canonical representative.  Expanding the first two exact-cover
 * choices creates root branches.  Branches with the same (possibly reflected)
 * residual mask are coalesced, retaining their exact multiplicity.  Dynamic
 * scheduling and the final multiplicity-weighted sum are exact regardless of
 * task order or thread count.
 *
 * MAX_N=12 uses 48 mask bits.  Every answer is at most the unrestricted
 * number of partitions into four-sets, (4*n)!/(24^n*n!).  This bound has 120
 * bits at n=12, so unsigned __int128 is sufficient throughout the supported
 * range.  Every accumulation is checked as an additional guard.
 *
 * Build:
 *   clang -O3 -march=native -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       397573_02.c -o 397573_02
 *
 * Results are atomically recorded in b397573_02.txt by default.  A --term
 * request is rejected before calculation if earlier b-file terms are missing;
 * use the prefix form or --no-bfile in that case.  No unfinished term is ever
 * written.
 *   ./397573_02 7 --threads 8
 *   ./397573_02 --term 8 --threads 8 --hash-power 19 --no-bfile
 *   ./397573_02 --check --threads 8 --no-bfile
 *
 * Cache memory is approximately 24*2^P bytes per active worker.  P=19 and
 * eight workers use about 96 MiB.  Lower P is always exact and safe but may be
 * slower because it retains fewer states.
 */
...
#error "397573_02.c requires unsigned __int128"
#endif
...
static uint64_t canonical_mask(uint64_t mask)
{
    const uint64_t reflected = reverse64(mask) >> (64U - active_values);
    return reflected < mask ? reflected : mask;
}
...
static void record_term(int n, U128 value)
{
    ...
    free(temporary);
    unlock_bfile(lock_fd);
    fprintf(stderr, "397573_02: recorded computed term n=%d in %s\n",
            n, output_path);
}
                     ...
                     uint64_t mask)
{
    ...
    schedule->tasks[schedule->count].mask = canonical_mask(mask);
...
static void *monitor_main(void *argument)
{
    ...
    while (!monitor_finished) {
        ...
        if (!monitor_finished && error == ETIMEDOUT) {
            fprintf(stderr,
                    "397573_02: heartbeat n=%d, tasks=%u/%u, "
                    "calls>=%" PRIu64 ", %.1f s\n",
                    monitor->n,
...
static U128 sequence_term(int n)
{
    ...
    fprintf(stderr,
            "397573_02: n=%d, exact-cover min-column, rows=%u, "
...
int main(int argc, char **argv)
{
    ...
    if (check) fprintf(stderr, "397573_02: self-check passed\n");
    return EXIT_SUCCESS;
}

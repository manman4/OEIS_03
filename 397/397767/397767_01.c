/*
 * A397767 -- deliberately naive independent counter.
 *
 * Count partitions of {1,...,4*n} into unordered four-element blocks that
 * can be split into two pairs having the same prime sum.  If a block is
 * written a < b < c < d, the first two possible pairings are strictly
 * unequal:
 *
 *     a+b < c+d,             a+c < b+d.
 *
 * Consequently the block is valid exactly when a+d = b+c is prime, and that
 * split is unique.  generate_rows() lists every such four-set once.
 *
 ...
 * independent check for small n rather than as the fastest A397767 program.
 *
 ...
 *       397767_01.c -o 397767_01
 *
 * Usage:
 *   ./397767_01 5
 *   ./397767_01 --term 5
 *   ./397767_01 --check --no-bfile
 *
 * OEIS offset is 1.  A positional N computes a(1),...,a(N).  By default,
 * completed terms are atomically recorded in b397767_01.txt.  --term N is
 * rejected before the search if earlier b-file terms are missing.  An
 * interrupted search never records the unfinished term.
 */
...
#error "397767_01.c requires unsigned __int128"
#endif
...
static const char *output_path = "b397767_01.txt";
...
static void record_term(int n, U128 value)
{
    ...
    free(temporary);
    unlock_bfile(lock_fd);
    fprintf(stderr, "397767_01: recorded computed term n=%d in %s\n",
            n, output_path);
}
...
static void heartbeat(void)
{
    ...
    fprintf(stderr, "397767_01: heartbeat calls=%" PRIu64 ", %.1f s\n",
            calls, elapsed);
...
static U128 sequence_term(int n)
{
    ...
    fprintf(stderr,
            "397767_01: n=%d, naive smallest-element recursion, "
...
int main(int argc, char **argv)
{
    ...
    if (check) fprintf(stderr, "397767_01: self-check passed\n");
    return EXIT_SUCCESS;
}

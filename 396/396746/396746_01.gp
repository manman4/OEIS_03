...
my(N=20, x='x+O('x^N)); Vec(serlaplace(1 - exp(-log(1-lambertw(lambertw(lambertw(-x))))) ))

\\ Expansion of e.g.f. -W_3(-x)/(1 - W_3(-x)), where W_k(x) is the k-th iterate of LambertW(x).
my(N=20, x='x+O('x^N)); Vec(serlaplace(1 - exp(-log(1-lambertw(lambertw(lambertw(-x))))) + lambertw(lambertw(lambertw(-x)))/(1-lambertw(lambertw(lambertw(-x)))) ))
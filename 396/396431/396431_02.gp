\\ A(n,k) = [x^n] F_k(x)/x and F_k(x) is the k-th iteration of x*G4(x)^3 with G4(x) = 1 + x*G4(x)^4.
T4(n,k) = {
...
\\ G.f. of column k: (1/x) * Series_Reversion( H_k(x) ), where H_k(x) is the k-th iteration of x*(1 - x)^3.
S4(n,k) = {

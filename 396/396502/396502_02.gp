\\ Square array A(n,k), n>=0, k>=0, read by antidiagonals downwards, where A(n,k) = n! * [x^n] (F_k(x)/x)^(1/2) and F_k(x) is the k-th iteration of x*G(x)^2 with G(x) = exp(x*G(x)^4).
Egen(n,k) = {
  ...
  n! * polcoef((F_iter/x)^(1/2), n)
};
matrix(6, 6, n, k, Egen(n-1, k-1))
for(n=0, 9, for(k=0, n, print1(Egen(k, n-k),", ")));

\\ E.g.f. of column k: ((1/x) * Series_Reversion( H_k(x) ))^(1/2), where H_k(x) is the k-th iterate of U(x)*exp(-4*U(x)) and U(x) = -LambertW(-2*x)/2.
Tgen(n,k) = {
  my(N = n+1, x = 'x + O('x^(N+1)));
  ...
  n! * polcoef((serreverse(Finv_k)/x)^(1/2), n)
};

...
\\ A(n,k) = [x^n] F_k(x)/x and F_k(x) is the k-th iteration of x*G3(x) with G3(x) = 1 + x*G3(x)^3.
T3(n,k) = {
...
\\ G.f. of column k: (1/x) * Series_Reversion( H_k(x) ), where H_k(x) is the k-th iteration of x*(1 - x*C(x)) with C(x) = 1 + x*C(x)^2.
S3(n,k) = {

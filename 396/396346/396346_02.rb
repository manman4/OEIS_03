...
# Verify: values other than 0 or those satisfying seq[n]==seq[n+2] appear only once
count = Hash.new(0)
seq.each { |v| count[v] += 1 }

errors = []
count.each do |val, cnt|
  next if cnt == 1
  next if val == 0
  next if output_seen[val]
  errors << "value #{val} appears #{cnt} times"
end

...
static void compute_and_write(int nmax, const char *out_path) {
  ...
  for (int n = 1; n <= nmax; ++n) {
    ...
    for (int s_limit = 1; s_limit <= n + 1; ++s_limit) {
      int s_add = s_limit - 1;
      if (s_add >= 1) {
        IntVec ds = divs[s_add];
        int n2 = n - s_add;
        ...
        for (int i = 0; i < ds.len; ++i) {
          ...
          if (n2 == 0) {
            ...
            BigRow prev = dp_sparse[n2][s_add];
            if (prev.a) {
      ...
      if (s_limit <= n) {
        ...
        for (int i = 0; i < row.len; ++i) {
          int d_prev = ds.v[i];
          mpz_init(row.a[i]);
          fenwick_sum(&fw, d_prev, pref);
          mpz_sub(row.a[i], active_total, pref);
        }
        dp_sparse[n][s_limit] = row;
      } else {
        for (int d_prev = 1; d_prev <= n + 1; ++d_prev) {
          fenwick_sum(&fw, d_prev, pref);
          mpz_sub(dp_full[n][d_prev], active_total, pref);
        }

        char *sval = mpz_get_str(NULL, 10, active_total);
...
int main(int argc, char **argv) {
  int nmax = 5000;
  const char *out_path = "b304430_1.txt";

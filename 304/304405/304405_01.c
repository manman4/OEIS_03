    ...
    for (int s = 1; s <= n; ++s) {
      ...
      if (s < n) {
        ...
        for (int i = 0; i < row.len; ++i) {
          int d_limit = ds.v[i];
          mpz_init(row.a[i]);
          fenwick_sum(&fw, d_limit - 1, row.a[i]);
        }
        dp_sparse[n][s] = row;
      } else {
        for (int d_limit = 1; d_limit <= n; ++d_limit) {
          fenwick_sum(&fw, d_limit - 1, dp_full[n][d_limit]);
        }
        mpz_set(dp_full[n][n + 1], active_total);

        char *sval = mpz_get_str(NULL, 10, dp_full[n][n + 1]);
...
int main(int argc, char **argv) {
  int nmax = 1000;
  const char *out_path = "b304405_1.txt";

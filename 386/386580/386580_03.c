 ...
 *
 * 出力: b386580_03.txt（OEIS の b-file 形式 "n a(n)"）
 *
 * コンパイルと実行
 *   cc -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *      386580_03.c -o 386580_03
 *   ./386580_03 [N]        # n = 0..N（既定 N = 100, 最大 127）
 *   ./386580_03 N --output FILE
 *   ./386580_03 N --no-bfile
 *
 ...
 *     これを繰り返せば r <= O + 1 にできる。）
 *
 * 3. run 長上限による 0/1 frontier DP（386580_02 とは異なる方式）
 ...
 *   が最終状態であり、その重み付き和が a(x) となる。386580_02 のような
 *   μ(λ) の保存、和 n ごとの再生成、現在層だけのハッシュ表は使用しない。
 *
...
#error "386580_03.c requires unsigned __int128"
#endif
...
static const char *output_path = "b386580_03.txt";
...
static u128 sequence_value[MAXN + 1];
...
static void offer(int n, const int *p, int t)
{
    if (set_insert(&layer[n], encode(p, t))) {
        ...
        if (sequence_value[n] > U128_MAX_VALUE - addend)
            die("sequence-value overflow");
        sequence_value[n] += addend;
...
int main(int argc, char **argv)
{
    ...
    offer(0, NULL, 0);
    compact_layer(0);
    verify_known(0, sequence_value[0]);
    record_term(0U, sequence_value[0]);
    {
        char number[40];
        u128_text(sequence_value[0], number);
        if (fprintf(stderr,
                    "0 %s  (%.2f s, partitions %zu, stored %zu, "
                    "transitions=%" PRIu64 ")\n",
    ...
    for (int x = 1; x <= nmax; ++x) {
        ...
        const u128 value = sequence_value[x];
        const u128 upper = (u128)1U << (x - 1);
        if (value > upper)
            die("sequence value exceeds the number of compositions");

 ...
 *
 * In a qualifying word, sort the distinct run lengths increasingly and
 * group them according to the letter used by each run.  The result is a
 ...
 * collisions cannot affect the answer.
 *
 * --check additionally uses a definition-level oracle.  It enumerates all
 * compositions through n=10 and recursively constructs runs, choosing a
 * letter different from the previous one and a length not used before.
 * This oracle does not use strict partitions, set partitions, the balance
 ...
 *
 * Build:
 *   cc -O3 -std=c11 -Wall -Wextra -Wpedantic 386580_01.c -o 386580_01
 *
 * Usage:
 *   ./386580_01                    # a(0),...,a(15)
 *   ./386580_01 20
 ...
 *   ./386580_01 --check 15
 *
 * Each newly computed term is recorded immediately in b386580_01.txt.
...
static const char *output_path = "b386580_01.txt";
static bool write_bfile = true;

#if SIZE_MAX < UINT64_MAX
#error "386580_01.c requires a 64-bit size_t"
#endif
...
static const uint64_t known_terms[KNOWN_MAX_N + 1] = {
    UINT64_C(1), UINT64_C(1), UINT64_C(1), UINT64_C(3),
    UINT64_C(3), UINT64_C(5), UINT64_C(12), UINT64_C(13),
    UINT64_C(20), UINT64_C(27), UINT64_C(64), UINT64_C(71),
    UINT64_C(108), UINT64_C(145), UINT64_C(206), UINT64_C(412)
};
...
static uint64_t count_a386580(unsigned n, Statistics *statistics)
{
    if (n == 0U) {
        if (statistics != NULL) {
    ...
    const uint64_t answer = (uint64_t)search.answers.size;
    if (statistics != NULL) {
...
static void record_term(unsigned n, uint64_t value)
{
    ...
    free(temporary);
    unlock_bfile(lock_descriptor);
    fprintf(stderr, "386580_01: recorded n=%u in %s\n", n, output_path);
}
...
static int check_implementation(unsigned maximum_n)
{
    for (unsigned n = 0U; n <= maximum_n; ++n) {
        ...
        uint64_t generated;
        if (n == 0U) {
            generated = UINT64_C(1);
        } else {
            generate_answers(n, &search);
            generated = (uint64_t)search.answers.size;
        }
        ...
        if (n <= ORACLE_MAX_N) {
            ...
            for (uint64_t separators = 0U;
                ...
                const bool oracle = oracle_has_composition(n, separators);
                const bool generator = n == 0U ? true :
    ...
    printf("ok: A386580 generator agrees with built-in terms for n=0..%u "
           "and direct run search for n=0..%u\n",
...
static int print_term(unsigned n)
{
    ...
    if (write_bfile && n < prefix) {
        answer = saved[n];
        fprintf(stderr, "386580_01: loaded n=%u from %s\n", n, output_path);
    } else {
        if (write_bfile && n > prefix) {
            fprintf(stderr,
        ...
        Statistics statistics;
        answer = count_a386580(n, &statistics);
        verify_known(n, answer);
        record_term(n, answer);
        fprintf(stderr,
                "386580_01: n=%u, strict partitions=%" PRIu64

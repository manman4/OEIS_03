 ...
 * checks the mutually verified range n=0..14.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       322180_01.c -o 322180_01
 *
 * Usage:
 *   ./322180_01             # print n=0..9
 *   ./322180_01 10          # print n=0..10
 *   ./322180_01 --term 9    # print a(9)
 *   ./322180_01 --check     # verify built-in terms
 */
...
#error "322180_01.c requires unsigned __int128"
#endif
    ...
    for (unsigned placed = 0; placed < (unsigned)n; ++placed) {
        ...
        for (;;) {
            ...
            if (ways != 0) {
                ...
                for (unsigned i = 0; i + 1 < positions; ++i) {
                    ...
                    const unsigned last =
                        i + (unsigned)label + 1 < positions ?
                        i + (unsigned)label + 1 : positions - 1;
                    const uint64_t above_first =
                        full & ~((UINT64_C(1) << (i + 1)) - 1);
                    const uint64_t through_last = last + 1 == positions ?
                        full : (UINT64_C(1) << (last + 1)) - 1;
                    uint64_t seconds =

/*
 * A322281 -- exact sparse subset DP with a three-value frontier.
 *
 ...
 *             1 <= j <= 3,  1 <= i <= n-j.
 *
 * If a valid prefix ends in a,b,c, appending d creates new constraints only
 * against a,b,c.  Thus the exact state is (used values,last three values):
 *
 *   D[S union {d},b,c,d] += D[S,a,b,c]
 *
 * for every unused d whose distance from a,b,c is at least 4.  A bit mask
 * represents S, and the permitted-d mask for every ordered triple is
 * precomputed.  Only reachable states are stored in two hash-table layers.
 * The worst-case bounds are O(n^4*2^n) time and O(n^3*2^n) space, with far
 ...
 * Every ordered-triple state has orbit size two: a fixed state would require
 * three distinct values to equal their complements componentwise.  Counts of
 ...
 *   - b322281_01_part.txt is flushed after every completed term and renamed
 *     atomically to b322281_01.txt only after successful completion.
 *
 * Verification:
 *   - OEIS b-file terms a(0)..a(21) are built in;
 *   - --check compares a direct permutation scan through n=10;
 *   - --check compares symmetry and no-symmetry DP through n=13;
 ...
 *     322281_01.c -o 322281_01
 *
 * Usage:
 *   ./322281_01                    # write a(0)..a(21)
 *   ./322281_01 --upto 19
 *   ./322281_01 --term 19 --memory-mib 2048
 *   ./322281_01 --no-symmetry --term 13
 *   ./322281_01 --check
 */
...
#error "322281_01 requires compiler support for unsigned __int128"
#endif
...
/* A322281, n=0..21, from https://oeis.org/A322281/b322281.txt. */
...
/*
 * Nearly all state coefficients fit in 64 bits, including the n=22 run.
                           ...
                           uint64_t memory_mib, DpStats *stats)
{
    ...
    for (int length = 3; length < n && current.size != 0U; ++length) {
        ...
        if (getenv("A322281_LAYER_STATS") != NULL)
            fprintf(stderr,
        ...
        for (size_t index = 0; index < current.capacity; ++index) {
            ...
            while (candidates != 0) {
                ...
                uint64_t key = canonical_state(new_used, b, c, d, n,
                                               use_symmetry);
                              ...
                              uint64_t memory_mib, bool verbose)
{
    ...
    if (verbose) {
        fprintf(stderr,
                "322281_01: n=%d, %s, peak states=%zu, "
...
static int check_implementation(int maximum_n, uint64_t memory_mib)
{
    ...
    printf("ok: A322281 b-file verified through n=%d; independent full "
...
{
    char *final_path =
        path_beside_executable(argv0, "b322281_01.txt");
    char *part_path =
        path_beside_executable(argv0, "b322281_01_part.txt");
    ...
    for (int n = SEQUENCE_OFFSET; n <= maximum_n; ++n) {
        ...
        if (fprintf(stream, "%d ", n) < 0 ||
            ...
            free(final_path);
            free(part_path);
            die("could not write the A322281 output file");
        }
    }
    if (fclose(stream) != 0) {
        free(final_path);
        free(part_path);
        die("could not close the A322281 output file");
    }
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            " MiB DP-table budget.\n"
            "A range run writes b322281_01.txt beside the executable.\n"

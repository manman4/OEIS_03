/*
 * A399215, version 02: least nonnegative k having exactly n
 ...
 * otherwise.  The number of positive solutions is
 *
 *   R(k) = (D(k)-A(k))/2.
 *
 ...
 *                     5 | a(n) unless n = 1.
 *
 * The exceptional value a(1)=7 is installed directly.  For every n>=2,
 * candidates are constructed explicitly as 5*u: the exponent assigned to
 * the least nonprincipal split prime 5 must be positive.  This removes every
 * candidate that the theorem proves cannot be minimal while retaining every
 * possible answer.  The self-test also runs the unrestricted version-01
 * search and checks equality through n=100.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 399215_02.c \
 *       -o 399215_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./399215_02 --target 23 --verbose
 *   ./399215_02 --upto 100
 *   ./399215_02 --self-test 1000000
 */
...
#define BFILE_NAME "b399215_02.txt"
#define BFILE_TEMP_NAME "b399215_02.txt.tmp"
                                      ...
                                      bool use_divisibility_theorem)
{
    ...
    } else if (use_divisibility_theorem && n == 1) {
        search.best.found = true;
        search.best.d = 2;
        search.best.axis = false;
        search.best.correction_two = false;
        search.best.prime_count = 1;
        search.best.primes[0] = 7;
        search.best.exponents[0] = 1;
        mpz_set_ui(search.best.value, 7);
    } else {
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 1; 6 * y * y <= limit; ++y) {
    ...
    static const Regression known_regressions[] = {
        {0, 0}, {1, 7}, {2, 55}, {3, 175}, {4, 385}, {5, 4375},
        {6, 2695}, {7, 30625}, {8, 9625}, {9, 21175}, {10, 96250},
        {12, 55825}, {13, 148225}, {15, 529375}, {16, 298375},
        {18, 390775}
    };
    for (size_t i = 0;
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (n == 1)
            passed = passed && mpz_cmp_ui(answer.value, 7) == 0;
        else
            passed = passed &&
                     mpz_divisible_ui_p(answer.value, 5) != 0;

/*
 * A399568 -- permutations whose interior terms are nondivisors.
 *
 * Count permutations p of [n] such that
 *
 *     p(j) does not divide Sum_{i=1..j} p(i),   2 <= j < n.
 *
 * Equivalently, p(j) does not divide the sum of its preceding terms.
 * The conditions at the first and last positions are both omitted.  Thus
 * the empty permutation is counted and a(0)=1.
 *
 * For a nonempty subset S of [n], let dp[S] count valid orders of S as an
 * initial segment.  If 2 <= |S| < n, its last element x is allowed exactly
 * when x does not divide sum(S), and
 *
 *     dp[S] = Sum_{x in S, sum(S) mod x != 0} dp[S \ {x}].
 *
 * Singletons have value 1.  For S=[n], all choices of the last element are
 * allowed because the condition stops at n-1.
 *
 * Only two adjacent subset-cardinality layers are retained.  Subsets are in
 * colexicographic order, where {b_1<...<b_k} has zero-based rank
 * Sum_i binomial(b_i,i).  Removing one element therefore gives an O(1)
 * lookup after O(k) prefix/suffix rank preparation.  The algorithm uses
 * O(n*2^n) arithmetic operations and O(binomial(n,floor(n/2))) memory.
 * At n=30 its maximum two-layer allocation is 4,808,643,120 bytes
 * (about 4.48 GiB), below the 6 GiB program limit.
 *
 * --upto writes a(0),...,a(MAX_N) atomically to b399568_01.txt and also
 * prints its b-file lines to standard output.  --check compares the DP with
 * an independent left-to-right exhaustive permutation search for n<=10.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       399568_01.c -o 399568_01
 *
 * Usage:
 *   ./399568_01                 # save a(0)..a(20)
 *   ./399568_01 22
 *   ./399568_01 --upto 22
 *   ./399568_01 --term 22
 *   ./399568_01 --check
...
#error "399568_01.c requires unsigned __int128"
#endif
...
#define BFILE_PATH "b399568_01.txt"
#define PART_PATH "b399568_01.txt.part"
...
static bool binomial_ready;

static void prepare_binomial(void)
{
    if (binomial_ready) {
        return;
    }
...
static U128 count_dp(unsigned n)
{
    ...
    prepare_binomial();
    uint64_t previous_count = binomial[n][1U];
    U128 *previous = allocate_layer(previous_count, n, 1U);
    for (uint64_t i = 0U; i < previous_count; ++i) {
        previous[i] = 1U;
    }

    for (unsigned cardinality = 2U; cardinality < n; ++cardinality) {
        const uint64_t current_count = binomial[n][cardinality];
        const uint64_t simultaneous_bytes =
            (previous_count + current_count) * sizeof(U128);
        if (simultaneous_bytes > MEMORY_LIMIT_BYTES) {
            free(previous);
        ...
        for (uint64_t index = 0U; index < current_count; ++index) {
            ...
            for (unsigned i = 0U; i < cardinality; ++i) {
                prefix_rank[i + 1U] =
                    prefix_rank[i] + binomial[positions[i]][i + 1U];
            }

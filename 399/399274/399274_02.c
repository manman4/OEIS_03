/*
 * A399274 (draft): least nonnegative k having exactly n nonnegative
 * integer solutions (x,y) satisfying
 *
 *                         x^2 + 9 y^2 = k.
 *
 * This is an exact prime-exponent search, not a scan up to a guessed bound.
 * Let U(m) count ordered nonnegative pairs (u,v) with u^2+v^2=m. Then
 *
 *   R_9(k) = U(k/9)  if 9 divides k,
 *            U(k)/2  if k == 1 (mod 3),
 *            0       otherwise.
 *
 ...
 *
 * and let S(m)=1 for a square and 0 otherwise. Then U(m)=D(m)+S(m)
 * for m>0, while U(0)=1. Thus the 9*u branch searches D=n
 * (nonsquare) and D=n-1 (square). The k==1 (mod 3) branch searches
 * D=2*n (nonsquare) and D=2*n-1
 * (square).
 *
 ...
 * Hence no possible minimum is omitted. In addition, the proved theorem
 *
 *                         5 | a(n) for n > 2
 *
 * is used as an exact pruning rule. For n>=3, the 9*u branch retains only
 * exponent patterns containing the prime 5, and the congruence branch
 * retains only assignments containing the prime 5. The theorem guarantees
 * that this cannot discard the true minimum.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 399274_02.c \
 *       -o 399274_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./399274_02 --target 23 --verbose
 *   ./399274_02 --upto 100
 *   ./399274_02 --self-test 1000000
 */
...
#define BFILE_NAME "b399274_02.txt"
#define BFILE_TEMP_NAME "b399274_02.txt.tmp"
...
static void store_if_better(Search *search, const mpz_t value, uint64_t d,
                            ...
                            unsigned prime_count)
{
    if (search->require_multiple_of_five &&
        mpz_divisible_ui_p(value, 5U) == 0)
...
static void consider_scaled_nine(Search *search,
                                 ...
                                 bool axis, bool correction_two)
{
    /* split_primes[0]=5, so this is exactly the condition 5|k. */
    if (search->require_multiple_of_five &&
                                ...
                                unsigned nonprincipal_parity)
{
    ...
    /* nonprincipal_primes[0]=5; reject before making any GMP products. */
    if (assignment->search->require_multiple_of_five &&
...
static void calculate_answer(uint64_t n, Answer *answer)
{
    ...
    search.require_multiple_of_five = n >= 3;
...
static uint64_t nonnegative_two_square_count(uint64_t k)
{
    if (k == 0) return 1;
...
{
    if (k == 0) return 1;
    if (k % 9 == 0) return nonnegative_two_square_count(k / 9);
    if (k % 3 == 1) return nonnegative_two_square_count(k) / 2;
    return 0;
}
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 0; 9 * y * y <= limit; ++y) {
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (passed && n >= 3)
            passed = mpz_divisible_ui_p(answer.value, 5U) != 0;
    ...
    printf("self-test passed: direct enumeration agrees for 0 <= k <= %"
           ...
           "regressions agree, the 5-divisibility theorem holds, and exact "

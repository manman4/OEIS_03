/*
 * A399202: least k having exactly n ordered nonnegative solutions of
 *
 *                    x^2 + y^2 = k.
 *
 ...
 *
 * Put D(k) = product (e_p+1), over primes p == 1 (mod 4).  If k>0 is
 ...
 * through all k below the answer.  The exceptional initial values are
 * a(0)=3 and a(1)=0.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 399202_01.c \
 *       -o 399202_01 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./399202_01 --target 23 --verbose
 *   ./399202_01 --upto 100
 *   ./399202_01 --self-test 1000000
 */
...
#define BFILE_NAME "b399202.txt"
#define BFILE_TEMP_NAME "b399202.txt.tmp"
...
static void calculate_answer(uint64_t n, Answer *answer)
{
    ...
    if (n == 0) {
        search.best.found = true;
        mpz_set_ui(search.best.value, 3);

 ...
 *
 * Build:
 *   cc -O3 -std=c11 -Wall -Wextra -Wpedantic 399206_02.c -o 399206_02
 *
 * Examples:
 *   ./399206_02                 # n=0..20; also updates b399206_02.txt
 *   ./399206_02 18              # n=0..18
 *   ./399206_02 --target 20     # only a(20); earlier b-file prefix required
 *   ./399206_02 20 --no-bfile   # stdout only
 *   ./399206_02 20 --output FILE
 *   ./399206_02 --self-test 10  # compare with direct permutation search
 *
 * The accepted representational limit is 33 because 33! fits in signed
 * 128-bit arithmetic.  Runtime grows quickly; 20 is the practical default.
...
#error "399206_02.c requires unsigned __int128"
#endif
...
static const char *output_path = "b399206_02.txt";
...
static u128 calculate(unsigned n, uint64_t *nodes, uint64_t *leaves,
                      unsigned *repeated_colors)
{
    ...
    inclusion_exclusion(&search, 0U, 1);
...
static void run_self_test(unsigned maximum)
{
    ...
    for (unsigned n = 0U; n <= maximum; ++n) {
        ...
        unsigned colors;
        const u128 fast = calculate(n, &nodes, &leaves, &colors);
        const u128 direct = calculate_direct(n);
        if (fast != direct) {
            fprintf(stderr, "self-test mismatch at n=%u: inclusion-exclusion=",
                    n);
...
int main(int argc, char **argv)
{
    ...
    for (unsigned n = first; n <= maximum; ++n) {
        ...
        uint64_t nodes, leaves;
        unsigned colors;
        const u128 value = calculate(n, &nodes, &leaves, &colors);
        record_term(n, value);
        print_term(n, value);
        if (fflush(stdout) != 0)

/*
 * A399267 (draft): least nonnegative k having exactly n nonnegative
 * integer solutions (x,y) of
 *
 *                         x^2 + 8 y^2 = k.
 *
 * This is an exact prime-exponent search, not a scan up to a guessed bound.
 * Let R_8(k) count solutions with x,y>=0 and define R_2 similarly for
 * x^2+2y^2.  Including axis solutions, the exact reductions are
 *
 *   odd:  k == 1 (mod 8), and R_8(k)=R_2(k);
 *   even: k=4*u,             and R_8(k)=R_2(u).
 *
 * All other positive k have R_8(k)=0.  For a represented u, put
 *
 *   D(u) = product (e_p+1),  p == 1 or 3 (mod 8),
 *
 * and let A(u)=1 when u is a square or twice a square, otherwise 0.
 * Then the number of nonnegative solutions is
 *
 *                         R_2(u)=(D(u)+A(u))/2.
 *
 * Hence R_8(k)=n permits only D=2*n with A=0, or D=2*n-1
 * with A=1.  A multiplicative partition D=product(e_i+1) enumerates
 * every exponent multiset.
 *
 * For k=4*u, decreasing exponents are assigned to 3,11,17,19,... .
 * For odd k, every assignment to primes 1 or 3 (mod 8) having an even
 * exponent sum at the 3 (mod 8) primes is considered.  Within each residue
 * class, decreasing exponents go on increasing primes.  Powers of 2 and
 * even powers of inert primes can only increase a candidate.  This proves
 * that the finite search returns the true minimum.  In addition, the proved
 * divisibility theorem
 *
 *                         9 | a(n) for n >= 1
 *
 * is used as an exact pruning rule.  For n>=2, an unrestricted candidate is
 * retained only when the exponent of 3 is at least 2.  In the odd constrained
 * search, assignments are retained only when 3 is present with exponent at
 * least 2.  This cannot discard the minimum by the theorem.  The exceptional
 * initial values are a(0)=2 and a(1)=0.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 399267_02.c \
 *       -o 399267_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./399267_02 --target 23 --verbose
 *   ./399267_02 --upto 100
 *   ./399267_02 --self-test 1000000
 */
...
#define BFILE_NAME "b399267_02.txt"
#define BFILE_TEMP_NAME "b399267_02.txt.tmp"
...
{
    switch (source) {
    case SOURCE_INITIAL: return "exceptional initial value";
...
static void store_if_better(Search *search, const mpz_t value, uint64_t d,
                            ...
                            unsigned prime_count)
{
    if (search->require_multiple_of_nine &&
        mpz_divisible_ui_p(value, 9U) == 0)
        die("divisibility pruning invariant failed");
...
static void consider_unrestricted(Search *search,
                                  ...
                                  AnswerSource source)
{
    /* The first split prime is 3, so this is exactly the condition 9|k. */
    if (search->require_multiple_of_nine &&
        (exponent_count == 0 || exponents[0] < 2))
        return;

    mpz_t value, power;
    mpz_init_set_ui(value, factor_four ? 4U : 1U);
    mpz_init(power);
...
static void consider_assignment(AssignmentSearch *assignment)
{
    ...
    /* q_primes[0]=3; reject the assignment before any GMP products are made. */
    if (assignment->search->require_multiple_of_nine &&
        (assignment->q_count == 0 || assignment->q_exponents[0] < 2))
        return;
    ...
    for (unsigned i = 0; i < assignment->q_count; ++i) {
        mpz_ui_pow_ui(power,
                      ...
                      (unsigned long)assignment->q_exponents[i]);
        mpz_mul(value, value, power);
    }
...
static void calculate_answer(uint64_t n, Answer *answer)
{
    ...
    search.require_multiple_of_nine = n >= 2;
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 0; 8 * y * y <= limit; ++y) {
    ...
    /*
     * Independently obtained by direct enumeration through k=5000000.
     * This draft sequence has no accepted OEIS data to cite yet.
     */
    static const Regression known_regressions[] = {
        {0, 2}, {1, 0}, {2, 9}, {3, 81}, {4, 297}, {5, 1089},
        {6, 1881}, {7, 12393}, {8, 5049}, {9, 18513}, {10, 16929},
        {11, 88209}, {12, 31977}, {13, 1185921}, {14, 152361},
        {15, 166617}, {16, 207009}, {18, 543609}, {20, 287793},
        {21, 1499553}, {23, 2832489}, {24, 1055241},
    ...
    for (uint64_t n = 0; n <= GUARANTEED_UPTO; ++n) {
        ...
        if (n == 0 || n == 1) {
            ...
            mpz_init_set_ui(reconstructed, answer.factor_four ? 4U : 1U);
            mpz_init(power);
        ...
        if (passed && n >= 1)
            passed = mpz_divisible_ui_p(answer.value, 9U) != 0;
    ...
    printf("self-test passed: direct enumeration agrees for 0 <= k <= %"
           ...
           "regressions agree, and exact structural checks pass through "
           "n=%" PRIu64 "\n",

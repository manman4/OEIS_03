/*
 * A399488 -- exact sparse-layer permanent implementation.
 *
 * Count permutations p of {1,...,n} satisfying
 *
 *     lcm(i, p(i)) >= n
 *
 * for every i.  This is the permanent of the symmetric 0-1 matrix
 *
 *     A[i,j] = 1  iff  lcm(i,j) >= n.
 *
 * The primary algorithm is a sparse row-by-row subset DP.  A state is the
 * mask of columns already used.  Rows are initially sorted by degree; before
 * every layer, a deterministic sample of the live states chooses the
 * remaining row with the fewest outgoing transitions.  Only reached masks
 * are stored in open-addressed hash tables.
 *
 * Counts use as many fixed 64-bit limbs as required by the rigorous product
 * of row degrees.  Every addition checks its final carry, and the final GMP
 * integer is checked against the same bound.  --check also uses a separate
 * recursive matching algorithm, with modular passes and exact CRT recovery,
 * through n=16.  That slower algorithm has independent state representation
 * and arithmetic.
 *
 * Safety policy:
 ...
 *     default (override with A399488_MEMORY_MIB);
 ...
 *       399488_01.c -lgmp -o 399488_01
 *   ./399488_01 --check
 *   ./399488_01 --term 23
 *   ./399488_01 --term 28
 *   ./399488_01 28 24   # use built-in terms 0..23; compute 24..28
 *
 * The normal output file is b399488_01.txt beside the executable, regardless
...
#define INDEPENDENT_GRAPH_CHECK_MAX_N 16
...
{
    const char *text = getenv("A399488_MEMORY_MIB");
    ...
    if (text != NULL && *text != '\0') {
        ...
        if (errno != 0 || end == text || *end != '\0' ||
            ...
            fprintf(stderr,
                    "error: A399488_MEMORY_MIB must be in %" PRIu64
                    "..%" PRIu64 ": %s\n",
...
static void make_path_bound(mpz_t bound, const SparseRow *rows, int n)
{
    ...
    /*
     * For n >= 2, row 1 can only use column n, and column 1 can only be
     * used by row n.  Thus p(1)=n and p(n)=1, leaving at most (n-2)!
     * completions.  Taking the smaller rigorous bound reduces the number
     * of fixed limbs without relying on floating-point estimates.
     */
    mpz_t factorial_bound;
    mpz_init_set_ui(factorial_bound, 1);
    for (int factor = 2; factor <= n - 2; ++factor) {
        mpz_mul_ui(factorial_bound, factorial_bound,
                   (unsigned long)factor);
    }
    if (mpz_cmp(factorial_bound, bound) < 0) {
        mpz_set(bound, factorial_bound);
    }
                                      ...
                                      bool report, DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "399488_01: n=%d, bound=%zu bits, %zu limb%s, "
                "memory limit=%.2f GiB\n",
...
/*
 * Independent checker: recursive perfect-matching DP.
 *
 ...
 * (C,R).  For A399488 this is slower than the layer DP, so --check uses it
 * only on a deliberately small prefix.
 */
                                ...
                                uint32_t value)
{
    ...
    for (;;) {
        ...
        if (solver->report && memo->count >= solver->next_report) {
            fprintf(stderr,
                    "399488_01: n=%d memo=%zu calls=%" PRIu64
                    " allocated=%.2f GiB\n",
                    solver->n, memo->count,
                                  ...
                                  DpStats *stats)
{
    ...
    if (report) {
        fprintf(stderr,
                "399488_01: n=%d graph modular DP, p=%" PRIu32
                ", memory limit=%.2f GiB\n",
                n, modulus,
                (double)memory_budget /
                    (double)(UINT64_C(1) << 30));
...
static DpStatus permanent_graph_exact(mpz_t result, int n,
                                      uint64_t memory_budget,
                                      bool report, DpStats *stats)
{
    memset(stats, 0, sizeof(*stats));
    ...
    if (report) {
        fprintf(stderr,
                "399488_01: n=%d exact graph CRT, bound=%zu bits, "
    ...
    for (size_t index = 0; index < modulus_count; ++index) {
        if (report) {
            fprintf(stderr,
                    "399488_01: n=%d CRT pass %zu/%zu, p=%" PRIu32
                    "\n",
        ...
        if (report) {
            fprintf(stderr,
                    "399488_01: n=%d pass %zu/%zu ok, residue=%" PRIu32
                           ...
                           const DpStats *stats)
{
    ...
    if (status == DP_MEMORY_LIMIT ||
        ...
        fprintf(stderr,
                "Increase A399488_MEMORY_MIB only if the machine has "
                "enough physical RAM.\n");
    }
}
                            ...
                            uint64_t memory_budget, bool report)
{
    DpStats stats;
    DpStatus status =
        permanent_layer_exact(value, n, memory_budget, report, &stats);
    if (status != DP_OK) {
    ...
    if (report) {
        fprintf(stderr,
                "399488_01: n=%d done, peak states=%zu, "
...
static int check_known_terms(int max_n, uint64_t memory_budget)
{
    ...
    mpz_t graph_value;
    mpz_init(value);
    mpz_init(graph_value);
    for (int n = 0; n <= max_n; ++n) {
        compute_checked(value, n, memory_budget, false);
        if (n <= INDEPENDENT_GRAPH_CHECK_MAX_N) {
            DpStats graph_stats;
            DpStatus graph_status = permanent_graph_exact(
                graph_value, n, memory_budget, false, &graph_stats);
            if (graph_status != DP_OK) {
                report_failure(n, graph_status, memory_budget,
                               &graph_stats);
                mpz_clear(graph_value);
                mpz_clear(value);
                return EXIT_FAILURE;
            }
            if (mpz_cmp(value, graph_value) != 0) {
                gmp_fprintf(
                    stderr,
                    "error: layer and graph DP disagree at n=%d: "
                    "%Zd versus %Zd\n",
                    n, value, graph_value);
                mpz_clear(graph_value);
                mpz_clear(value);
    ...
    mpz_clear(graph_value);
    mpz_clear(value);
    printf("ok: layer DP agrees with the known A399488 prefix "
           "for n=0..%d; graph DP independently agrees for n=0..%d\n",
           max_n,
           max_n < INDEPENDENT_GRAPH_CHECK_MAX_N
               ? max_n : INDEPENDENT_GRAPH_CHECK_MAX_N);
    return EXIT_SUCCESS;
}
                           ...
                           uint64_t memory_budget)
{
    ...
    char *path = path_beside_executable(argv0, "b399488_01.txt");
    ...
    if (start_n > 0) {
        fprintf(stderr,
                "399488_01: using built-in verified prefix n=0..%d\n",
                start_n - 1);
    }

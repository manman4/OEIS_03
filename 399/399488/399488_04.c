 ...
 *
 * This program is an independent arithmetic/order verifier for 03:
 *
 *   399488_01.c  sparse subset DP on allowed column masks;
 *   399488_02.c  dense component rook DP on every reachable column mask;
 *   399488_03.c  min-fill-first tree decomposition and ~2.147e9 primes;
 *   399488_04.c  min-degree-first tree decomposition and disjoint
 *                ~9.970e8 primes.
 *
 ...
 * The deterministic min-degree order uses fill count only as a tie-breaker.
 * It has width at most 16 for n <= 60 in this problem (the program still
 * verifies a hard width limit at runtime), so the boundary tables remain far
 * smaller than 2^m arrays.
...
{
    const char *text = getenv("A399488_04_MEMORY_MIB");
    ...
    if (text != NULL && *text != '\0') {
        ...
        if (errno != 0 || end == text || *end != '\0' ||
            ...
            fprintf(stderr,
                    "error: A399488_04_MEMORY_MIB must be in %" PRIu64
                    "..%" PRIu64 ": %s\n",
...
static ComputeStatus min_degree_decomposition(
    ...
    for (unsigned step = 0; step < vertex_count; ++step) {
        ...
        while ((candidate = vertex_set_take_first(&scan)) >= 0) {
            ...
            if (best < 0 || degree < best_degree ||
                (degree == best_degree && fill < best_fill) ||
                (degree == best_degree && fill == best_fill &&
                 candidate < best)) {
                best = candidate;
                best_fill = fill;
    ...
    MemoryTracker *memory, bool report, ComputeStats *stats)
{
    ...
    ComputeStatus status = min_degree_decomposition(
    ...
    if (report) {
        fprintf(stderr,
                "399488_04: n=%d forbidden graph vertices=%u, "
                "min-degree width=%u\n",
    ...
    *global_degree = 0;
    ...
    for (unsigned step = 0; step < vertex_count; ++step) {
        ...
        if (report &&
            ...
            fprintf(stderr,
                    "399488_04: n=%d bags=%u/%u, active memory=%.2f "
                    "MiB\n",
                                   ...
                                   bool report, ComputeStats *stats)
{
    ...
    if (report && status == COMPUTE_OK) {
        fprintf(stderr,
                "399488_04: n=%d exact CRT, bound=%zu bits, "
                "%zu primes, width=%u\n",
                           ...
                           const ComputeStats *stats)
{
    ...
    if (status == COMPUTE_MEMORY_LIMIT ||
        ...
        fprintf(stderr,
                "Increase A399488_04_MEMORY_MIB only when enough "
                            ...
                            uint64_t memory_budget, bool report)
{
    ...
    if (report) {
        fprintf(stderr,
                "399488_04: n=%d done, width=%u, table pairs=%" PRIu64
...
static int check_known_terms(int max_n, uint64_t memory_budget)
{
    ...
    mpz_clear(value);
    printf("ok: min-degree tree DP with the independent prime set "
           "agrees with the known A399488 "
...
static void produce_b_file(const char *argv0, int max_n,
                           uint64_t memory_budget)
{
    char *path = path_beside_executable(argv0, "b399488_04.txt");
...
static void usage(const char *program)
{
    fprintf(stderr,
            ...
            "Every b-file term is computed by 399488_04 itself.\n"

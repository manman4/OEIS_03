    ...
    if (rest_size >= MITM_MIN_REST_SIZE) {
        ...
        for (;;) {
            ...
            for (unsigned target =
                ...
                const uint64_t prime_mask = context->prime_bit[target];
                if ((used_primes & prime_mask) != 0) continue;
        ...
        for (;;) {
            ...
            const uint64_t prime_mask = context->prime_bit[block_sum];
            if (prime_mask != 0 && (used_primes & prime_mask) == 0)
                checked_add(&total,
                                  ...
                                  size_t *result_count)
{
    ...
    for (;;) {
        const unsigned block_sum =
            pivot_value + subset_sum(context, block_rest);
        if (context->prime_bit[block_sum] != 0) ++total;
        if (block_rest == 0) break;
        block_rest = (block_rest - 1U) & rest;
    }
    ...
    for (;;) {
        ...
        if (prime_mask != 0) {
            ...
            tasks[used].prime_mask = prime_mask;
            ++used;
        }
        if (block_rest == 0) break;
        block_rest = (block_rest - 1U) & rest;
...
static void usage(FILE *stream, const char *program)
{
    fprintf(stream,
            ...
            "               n=20 uses about 168 MiB of memo storage\n"

/*
 * A399191, version 02: least k having exactly n positive solutions of
 *
 *                    x^2 + 4 y^2 = k.
 *
 ...
 * the number T(k) of all integer solutions is
 *
 *     T(k) = 2 D(k)   if e=0,
 *            0        if e=1,
 *            4 D(k)   if e>=2.
 *
 * Each positive solution has four independent sign choices.  There are two
 * signed solutions on the x-axis when k is a square, and two on the y-axis
 * when k/4 is a square.  Thus
 *
 *     R(k) = (T(k) - 2 A(k))/4,
 *
 * where A(k) is the number (0, 1, or 2) of those two axis conditions.
 * Consequently R(k)=n has exactly the following four types of candidates:
 *
 *   odd, nonsquare:        D=2n;
 *   odd, square:           D=2n+1;
 ...
 * etc.  For an even nonsquare candidate, the least 2-adic factor is 4 when
 * some e_i is odd and 8 when all e_i are even.  An even square candidate has
 * 2-adic factor 4.  Squares of primes == 3 (mod 4) only increase k.  Hence
 * the search proves minimality without relying on unpublished sequence data.
 *
 * For every n>0 the minimum contains the split prime 5.  Indeed, n>=2 forces
 * D>1, and a minimum must assign its largest positive exponent to the least
 * split prime 5.  For n=1 the least 5-free candidate is 8, whereas k=5 is
 * smaller.  Version 02 uses this theorem to discard every 5-free candidate.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic 399191_02.c \
 *       -o 399191_02 $(pkg-config --cflags --libs gmp)
 *
 * Examples:
 *   ./399191_02 --target 23 --verbose
 *   ./399191_02 --upto 100
 *   ./399191_02 --self-test 1000000
 */
...
#define BFILE_NAME "b399191_02.txt"
#define BFILE_TEMP_NAME "b399191_02.txt.tmp"
#define MANDATORY_SPLIT_PRIME UINT64_C(5)
...
{
    unsigned count = 0;
    for (uint64_t p = MANDATORY_SPLIT_PRIME; count < MAX_FACTORS; p += 2)
        if (p % 4 == 1 && is_prime(p))
            primes[count++] = p;
}
...
static void consider_partition(Search *search, uint64_t d,
                               CandidateType type)
{
    /* The proved divisibility 5 | a(n), n>0, excludes D=1 patterns. */
...
static void self_test(uint64_t limit)
{
    ...
    for (uint64_t y = 1; 4 * y * y <= limit; ++y)
        for (uint64_t x = 1; x * x <= limit - 4 * y * y; ++x)
            ++direct[x * x + 4 * y * y];
    ...
    printf("self-test passed: direct enumeration agrees for 0 <= k <= %"
           ...
           "initial values and 5-divisibility agree, and structural checks "
           "pass through n=%"

/*
 * A332748 -- ordinary-prime algebraic CRT implementation.
 *
 ...
 * modulo one, two, or three distinct ordinary primes near 2^59.  CRT lanes
 ...
 * n<=17, while B(18) exceeds their product.  Deterministic 64-bit
 * Miller-Rabin checks every configured prime at each invocation.  Generic
 * incremental CRT in a checked U256 reconstructs the answer.  Shoup
 * multiplication makes each small |Q_d| product exact without division in
 * the hot loop.  Invalid distances are omitted and their fixed bit masks are
 * prepared per worker; pthread workers use a dynamic atomic schedule.
 *
 * This is independent of 332748_01.c's Mersenne reduction and uses a
 * different modulus set.  The additional alternating-sign involution from
 * 322178_02.c is not used: for cubic triples it introduces the start-dependent
 * factor (-1)^(i+d), so Q_d does not transform by one scalar depending only
 * on d.  Reversal remains an exact two-element symmetry.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic -pthread \
 *       332748_02.c -o 332748_02
 *
 * Usage:
 *   ./332748_02 8 --threads 8
 *   ./332748_02 --term 12 --threads 8
 *   ./332748_02 --check --threads 8 --no-bfile
 *
 * Completed terms are atomically recorded in b332748_02.txt by default.
...
#error "332748_02.c requires unsigned __int128"
#endif
...
static const char *output_path = "b332748_02.txt";
...
static void store_bfile_term(int n, U256 value)
{
    ...
    free(temporary);
    release_bfile_lock(lock_fd);
    fprintf(stderr, "332748_02: recorded computed term n=%d in %s\n",
            n, output_path);
}
                                   ...
                                   const Modulus *modulus)
{
    ...
    /*
     * Exact 64-bit Shoup multiplication.  With
     * c=floor(multiplier*2^64/p), q=floor(value*c/2^64) is at most one
     * below floor(value*multiplier/p).  The unsigned low products may wrap,
     * but the wrap cancels in their subtraction.  The remainder is below
     * 2*p and fits in uint64_t because p<2^63; one subtraction suffices.
     */
                           ...
                           uint64_t residues[MAX_LANES])
{
    ...
    for (unsigned lane = 0; lane < lanes; ++lane) {
        if (moduli[lane] <= 1 || (moduli[lane] & 1) == 0 ||
            ...
            die("ordinary prime violates Shoup arithmetic preconditions");
        }
...
static U256 a332748(int n)
{
    ...
    fprintf(stderr,
            "332748_02: n=%d, ordinary-prime CRT, evaluations=%" PRIu64
...
static bool is_prime_u64(uint64_t value)
{
    ...
    /* This witness set is deterministic for every uint64_t input. */
...
static void verify_constants_and_paths(void)
{
    for (unsigned lane = 0; lane < MAX_LANES; ++lane) {
        if (moduli[lane] <= 1 || (moduli[lane] & 1) == 0 ||
            moduli[lane] >= (UINT64_C(1) << 63)) {
            die("configured prime violates Shoup preconditions");
        }
        if (!is_prime_u64(moduli[lane])) {
        ...
        for (unsigned multiplier = 0;
            ...
            if (mod_multiply_small(endpoint, multiplier, &test) !=
                endpoint_expected) {
                die("ordinary-prime Shoup endpoint self-test failed");
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (mode == MODE_CHECK) {
        ...
        printf("ok: A332748 n=0..%d and ordinary-prime CRT paths verified\n",
               CHECK_N);
        return EXIT_SUCCESS;
    }
    if (mode == MODE_TERM) {
